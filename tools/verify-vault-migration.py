...
def load_legacy_records(root, legacy_ref):
    """Read legacy directives and retain the annotations for each vault."""
    legacy = subprocess.check_output(
        ["git", "show", f"{legacy_ref}:lib/edit/VaultDefinitions.txt"], cwd=root
    ).decode("utf-8")
    records = []
    comments = []
    pending = []
...
def reflow_layouts(records):
    """Reflow records in place and return the original flat layouts for comparison."""
    expected = []
...
def render_jsonc(records, comments):
    """Render vault records with their legacy annotations."""
...
def write_formatted_jsonc(root, target, content):
    """Format successfully before replacing the definition file."""
    # A missing/failing formatter must not leave an unformatted definition behind.
    # Verification alone needs no Node.
    ...
    formatted = subprocess.check_output(
        ...
        input=content.encode("utf-8"), cwd=root,
    ).decode("utf-8")
    target.write_text(formatted, encoding="utf-8", newline="\n")


def verify_migration(target, expected):
    """Compare metadata and layout bytes, then check the JSONC row dimensions."""
...
def main():
    ...
    records, comments = load_legacy_records(root, args.legacy_ref)
    expected = reflow_layouts(records)
    if args.write:
        write_formatted_jsonc(root, target, render_jsonc(records, comments))
    verify_migration(target, expected)
