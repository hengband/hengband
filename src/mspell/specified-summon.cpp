...
MONSTER_NUMBER summon_LAFFEY_II(PlayerType *player_ptr, const Pos2D &position, MONSTER_IDX m_idx)
{
    ...
    auto real_count = summon_num - monraces_info[MonsterRaceId::BUNBUN_STRIKERS].cur_num;

    if (!floor_ptr->inside_arena && real_count < MAX_BUNBUN_NUM) {
        for (auto &monster : floor_ptr->m_list) {
            if (monster.r_idx == MonsterRaceId::BUNBUN_STRIKERS) {
                ...
                auto &current_grid = floor_ptr->get_grid(current_position);
                ...
                floor_ptr->get_grid(attract_position).m_idx = target_m_idx;
