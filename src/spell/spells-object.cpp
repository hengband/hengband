...
static amuse_type amuse_info[]
    = { { ItemPrimaryType::TV_BOTTLE, SV_ANY, 5, AMS_NOTHING }, { ItemPrimaryType::TV_JUNK, SV_ANY, 3, AMS_MULTIPLE }, { ItemPrimaryType::TV_SPIKE, SV_ANY, 10, AMS_PILE }, { ItemPrimaryType::TV_STATUE, SV_ANY, 15, AMS_NOTHING },
          { ItemPrimaryType::TV_CORPSE, SV_ANY, 15, AMS_NO_UNIQUE }, { ItemPrimaryType::TV_SKELETON, SV_ANY, 10, AMS_NO_UNIQUE }, { ItemPrimaryType::TV_FIGURINE, SV_ANY, 10, AMS_NO_UNIQUE },
          { ItemPrimaryType::TV_PARCHMENT, SV_ANY, 1, AMS_NOTHING }, { ItemPrimaryType::TV_POLEARM, SV_TSURIZAO, 3, AMS_NOTHING }, // Fishing Pole of Taikobo
          { ItemPrimaryType::TV_SWORD, SV_BROKEN_DAGGER, 3, AMS_FIXED_ART }, // Broken Dagger of Magician
          { ItemPrimaryType::TV_SWORD, SV_BROKEN_DAGGER, 10, AMS_NOTHING }, { ItemPrimaryType::TV_SWORD, SV_BROKEN_SWORD, 5, AMS_NOTHING }, { ItemPrimaryType::TV_SCROLL, SV_SCROLL_AMUSEMENT, 10, AMS_NOTHING },

          { ItemPrimaryType::TV_NONE, 0, 0, 0 } };
...
{
    int t = 0;
    for (int n = 0; amuse_info[n].tval != ItemPrimaryType::TV_NONE; n++) {
        t += amuse_info[n].prob;
    }
...
void brand_bolts(player_type *player_ptr)
{
    ...
    for (int i = 0; i < INVEN_PACK; i++) {
        ...
        /* Skip non-bolts */
        if (o_ptr->tval != ItemPrimaryType::TV_BOLT)
            continue;
...
bool enchant_equipment(player_type *player_ptr, object_type *o_ptr, int n, int eflag)
{
    ...
    if ((o_ptr->tval == ItemPrimaryType::TV_BOLT) || (o_ptr->tval == ItemPrimaryType::TV_ARROW) || (o_ptr->tval == ItemPrimaryType::TV_SHOT)) {
        prob = prob / 20;
    }
...
void brand_weapon(player_type *player_ptr, int brand_type)
{
    ...
    bool is_special_item = o_ptr->k_idx && !o_ptr->is_artifact() && !o_ptr->is_ego() && !o_ptr->is_cursed()
        && !((o_ptr->tval == ItemPrimaryType::TV_SWORD) && (o_ptr->sval == SV_POISON_NEEDLE)) && !((o_ptr->tval == ItemPrimaryType::TV_POLEARM) && (o_ptr->sval == SV_DEATH_SCYTHE))
        && !((o_ptr->tval == ItemPrimaryType::TV_SWORD) && (o_ptr->sval == SV_DIAMOND_EDGE));
    if (!is_special_item) {
        if (flush_failure)
            flush();
    ...
    switch (brand_type) {
    case 17:
        if (o_ptr->tval == ItemPrimaryType::TV_SWORD) {
