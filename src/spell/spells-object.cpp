...
bool enchant_spell(player_type *caster_ptr, HIT_PROB num_hit, HIT_POINT num_dam, ARMOUR_CLASS num_ac)
{
    ...
    ItemTester item_tester(object_allow_enchant_weapon);
...
void brand_weapon(player_type *caster_ptr, int brand_type)
{
    ...
    OBJECT_IDX item;
    object_type *o_ptr;
    o_ptr = choose_object(caster_ptr, &item, q, s, USE_EQUIP | IGNORE_BOTHHAND_SLOT, TV_NONE, ItemTester(object_allow_enchant_melee_weapon));
    if (!o_ptr)
        return;
