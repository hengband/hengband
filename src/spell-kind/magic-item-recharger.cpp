...
bool recharge(player_type *player_ptr, int power)
{
    ...
    if (o_ptr->tval == ItemPrimaryType::TV_ROD) {
        ...
        if ((o_ptr->tval == ItemPrimaryType::TV_WAND) && (o_ptr->number > 1))
        ...
        if (one_in_(recharge_strength)) {
            ...
            if ((o_ptr->tval == ItemPrimaryType::TV_STAFF) && (o_ptr->number > 1)) {
    ...
    if (o_ptr->is_fixed_artifact()) {
        ...
        if ((o_ptr->tval == ItemPrimaryType::TV_ROD) && (o_ptr->timeout < 10000))
            o_ptr->timeout = (o_ptr->timeout + 100) * 2;
        else if ((o_ptr->tval == ItemPrimaryType::TV_WAND) || (o_ptr->tval == ItemPrimaryType::TV_STAFF))
            o_ptr->pval = 0;
        return update_player(player_ptr);
    }
    ...
    if (PlayerClass(player_ptr).is_wizard()) {
        ...
        else if (o_ptr->tval == ItemPrimaryType::TV_WAND) {
            if (!one_in_(3))
                fail_type = 2;
            else
                fail_type = 1;
        ...
        else if (o_ptr->tval == ItemPrimaryType::TV_STAFF) {
            if (one_in_(2))
                fail_type = 2;
            else
                fail_type = 0;
