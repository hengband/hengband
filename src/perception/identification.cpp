...
bool screen_object(player_type *player_ptr, object_type *o_ptr, BIT_FLAGS mode)
{
    ...
    if (o_ptr->tval == ItemPrimaryType::TV_FIGURINE) {
    ...
    if ((o_ptr->tval == ItemPrimaryType::TV_POLEARM) && (o_ptr->sval == SV_DEATH_SCYTHE)) {
    ...
    if ((o_ptr->tval == ItemPrimaryType::TV_STATUE) && (o_ptr->sval == SV_PHOTO)) {
        monster_race *r_ptr = &r_info[o_ptr->pval];
