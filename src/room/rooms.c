...
bool find_space(player_type *player_ptr, POSITION *y, POSITION *x, POSITION height, POSITION width)
{
    ...
    if (dun->row_rooms < blocks_high)
        return FALSE;
    if (dun->col_rooms < blocks_wide)
        return FALSE;
...
static void cave_fill(player_type *player_ptr, POSITION y, POSITION x)
{
    ...
    while (flow_head_room != flow_tail_room) {
        ...
        for (d = 0; d < 8; d++) {
            ...
            /* If within bounds */
            else if ((i > fill_data.xmin) && (i < fill_data.xmax) && (j > fill_data.ymin) && (j < fill_data.ymax)) {
                /* If not a wall or floor done before */
                if (hack_isnt_wall(player_ptr, j, i, fill_data.c1, fill_data.c2, fill_data.c3, fill_data.feat1, fill_data.feat2, fill_data.feat3,
