...
bool build_type14(PlayerType *player_ptr, DungeonData *dd_ptr)
{
    ...
    for (auto y = y1 - 1; y <= y2 + 1; y++) {
        place_grid(floor, floor.get_grid({ y, x1 - 1 }), GB_OUTER);
        place_grid(floor, floor.get_grid({ y, x2 + 1 }), GB_OUTER);
    }

    for (auto x = x1 - 1; x <= x2 + 1; x++) {
        place_grid(floor, floor.get_grid({ y1 - 1, x }), GB_OUTER);
        place_grid(floor, floor.get_grid({ y2 + 1, x }), GB_OUTER);
    }
