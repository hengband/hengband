...
void place_pit_outer(PlayerType *player_ptr, const Pos2D &center)
{
    ...
    for (auto y = rectangle.top_left.y - 1; y <= rectangle.bottom_right.y + 1; y++) {
        place_grid(floor, floor.get_grid({ y, rectangle.top_left.x - 1 }), GB_OUTER);
        place_grid(floor, floor.get_grid({ y, rectangle.bottom_right.x + 1 }), GB_OUTER);
    }

    for (auto x = rectangle.top_left.x - 1; x <= rectangle.bottom_right.x + 1; x++) {
        place_grid(floor, floor.get_grid({ rectangle.top_left.y - 1, x }), GB_OUTER);
        place_grid(floor, floor.get_grid({ rectangle.bottom_right.y + 1, x }), GB_OUTER);
    }
}
...
void place_pit_inner(PlayerType *player_ptr, const Pos2D &center)
{
    ...
    for (auto y = rectangle.top_left.y - 1; y <= rectangle.bottom_right.y + 1; y++) {
        place_grid(floor, floor.get_grid({ y, rectangle.top_left.x - 1 }), GB_INNER);
        place_grid(floor, floor.get_grid({ y, rectangle.bottom_right.x + 1 }), GB_INNER);
    }

    for (auto x = rectangle.top_left.x - 1; x <= rectangle.bottom_right.x + 1; x++) {
        place_grid(floor, floor.get_grid({ rectangle.top_left.y - 1, x }), GB_INNER);
        place_grid(floor, floor.get_grid({ rectangle.bottom_right.y + 1, x }), GB_INNER);
    }
...
bool build_type13(PlayerType *player_ptr, DungeonData *dd_ptr)
{
    ...
    for (auto y = rectangle.top_left.y - 1; y <= rectangle.bottom_right.y + 1; y++) {
        for (auto x = rectangle.top_left.x - 1; x <= rectangle.bottom_right.x + 1; x++) {
            auto &grid = floor.get_grid({ y, x });
            place_grid(floor, grid, GB_INNER);
            grid.add_info(CAVE_ROOM);
        }
    }
    ...
    for (auto x = rectangle.top_left.x + 3; x <= rectangle.bottom_right.x - 3; x++) {
        ...
        place_grid(floor, grid_top, GB_FLOOR);
        grid_top.add_info(CAVE_ICKY);

        auto &grid_bottom = floor.get_grid({ center->y + 2, x });
        place_grid(floor, grid_bottom, GB_FLOOR);
    ...
    for (auto x = rectangle.top_left.x + 5; x <= rectangle.bottom_right.x - 5; x++) {
        ...
        place_grid(floor, grid_left, GB_FLOOR);
        grid_left.add_info(CAVE_ICKY);

        auto &grid_right = floor.get_grid({ center->y + 3, x });
        place_grid(floor, grid_right, GB_FLOOR);
        grid_right.add_info(CAVE_ICKY);
    }

    /* Corridor */
    for (auto x = rectangle.top_left.x; x <= rectangle.bottom_right.x; x++) {
        place_grid(floor, floor.get_grid({ center->y, x }), GB_FLOOR);
        place_grid(floor, floor.get_grid({ rectangle.top_left.y, x }), GB_FLOOR);
        place_grid(floor, floor.get_grid({ rectangle.bottom_right.y, x }), GB_FLOOR);
    }
