...
bool build_type1(PlayerType *player_ptr, DungeonData *dd_ptr)
{
    ...
    for (auto y = top - 1; y <= bottom + 1; y++) {
        place_grid(floor, floor.get_grid({ y, left - 1 }), GB_OUTER);
        place_grid(floor, floor.get_grid({ y, right + 1 }), GB_OUTER);
    }

    for (auto x = left - 1; x <= right + 1; x++) {
        place_grid(floor, floor.get_grid({ top - 1, x }), GB_OUTER);
        place_grid(floor, floor.get_grid({ bottom + 1, x }), GB_OUTER);
    }
    ...
    if (one_in_(20)) {
        if ((top + 4 < bottom) && (left + 4 < right)) {
            place_grid(floor, floor.get_grid({ top + 1, left + 1 }), GB_INNER);
            place_grid(floor, floor.get_grid({ top + 1, right - 1 }), GB_INNER);
            place_grid(floor, floor.get_grid({ bottom - 1, left + 1 }), GB_INNER);
            place_grid(floor, floor.get_grid({ bottom - 1, right - 1 }), GB_INNER);
        }
    ...
    if (one_in_(50)) {
        for (auto y = top + 2; y <= bottom - 2; y += 2) {
            place_grid(floor, floor.get_grid({ y, left }), GB_INNER);
            place_grid(floor, floor.get_grid({ y, right }), GB_INNER);
        }

        for (auto x = left + 2; x <= right - 2; x += 2) {
            place_grid(floor, floor.get_grid({ top, x }), GB_INNER);
            place_grid(floor, floor.get_grid({ bottom, x }), GB_INNER);
        }
...
bool build_type2(PlayerType *player_ptr, DungeonData *dd_ptr)
{
    ...
    for (auto y = y1a - 1; y <= y2a + 1; y++) {
        place_grid(floor, floor.get_grid({ y, x1a - 1 }), GB_OUTER);
        place_grid(floor, floor.get_grid({ y, x2a + 1 }), GB_OUTER);
    }
    for (auto x = x1a - 1; x <= x2a + 1; x++) {
        place_grid(floor, floor.get_grid({ y1a - 1, x }), GB_OUTER);
        place_grid(floor, floor.get_grid({ y2a + 1, x }), GB_OUTER);
    }

    /* Place the walls around room "b" */
    for (auto y = y1b - 1; y <= y2b + 1; y++) {
        place_grid(floor, floor.get_grid({ y, x1b - 1 }), GB_OUTER);
        place_grid(floor, floor.get_grid({ y, x2b + 1 }), GB_OUTER);
    }
    for (auto x = x1b - 1; x <= x2b + 1; x++) {
        place_grid(floor, floor.get_grid({ y1b - 1, x }), GB_OUTER);
        place_grid(floor, floor.get_grid({ y2b + 1, x }), GB_OUTER);
    }

    /* Replace the floor for room "a" */
    for (auto y = y1a; y <= y2a; y++) {
        for (auto x = x1a; x <= x2a; x++) {
            place_grid(floor, floor.get_grid({ y, x }), GB_FLOOR);
        }
    }
...
bool build_type3(PlayerType *player_ptr, DungeonData *dd_ptr)
{
    ...
    case 1: {
        for (const auto &pos : rect_inner) {
            place_grid(floor, floor.get_grid(pos), GB_INNER);
        }
        break;
    }
    ...
    case 3: {
        ...
        if (one_in_(3)) {
            ...
            for (auto y = y1b; y <= y2b; y++) {
                ...
                place_grid(floor, floor.get_grid({ y, x1a - 1 }), GB_INNER);
                place_grid(floor, floor.get_grid({ y, x2a + 1 }), GB_INNER);
            }
            ...
            for (auto x = x1a; x <= x2a; x++) {
                ...
                place_grid(floor, floor.grid_array[y1b - 1][x], GB_INNER);
                place_grid(floor, floor.grid_array[y2b + 1][x], GB_INNER);
            }
        ...
        /* Occasionally put a "plus" in the center */
            ...
            place_grid(floor, floor.get_grid(*center), GB_INNER);
            place_grid(floor, floor.get_grid({ y1b, center->x }), GB_INNER);
            place_grid(floor, floor.get_grid({ y2b, center->x }), GB_INNER);
            place_grid(floor, floor.get_grid({ center->y, x1a }), GB_INNER);
            place_grid(floor, floor.get_grid({ center->y, x2a }), GB_INNER);
        }
...
bool build_type4(PlayerType *player_ptr, DungeonData *dd_ptr)
{
    ...
    /* Outer Walls */
    for (auto y = y1_outer - 1; y <= y2_outer + 1; y++) {
        place_grid(floor, floor.get_grid({ y, x1_outer - 1 }), GB_OUTER);
        place_grid(floor, floor.get_grid({ y, x2_outer + 1 }), GB_OUTER);
    }
    for (auto x = x1_outer - 1; x <= x2_outer + 1; x++) {
        place_grid(floor, floor.get_grid({ y1_outer - 1, x }), GB_OUTER);
        place_grid(floor, floor.get_grid({ y2_outer + 1, x }), GB_OUTER);
    }
    ...
    for (auto y = y1_inner - 1; y <= y2_inner + 1; y++) {
        place_grid(floor, floor.get_grid({ y, x1_inner - 1 }), GB_INNER);
        place_grid(floor, floor.get_grid({ y, x2_inner + 1 }), GB_INNER);
    }
    for (auto x = x1_inner - 1; x <= x2_inner + 1; x++) {
        place_grid(floor, floor.get_grid({ y1_inner - 1, x }), GB_INNER);
        place_grid(floor, floor.get_grid({ y2_inner + 1, x }), GB_INNER);
    }
    ...
    case 2: {
        ...
        for (auto y = center->y - 1; y <= center->y + 1; y++) {
            for (auto x = center->x - 1; x <= center->x + 1; x++) {
                ...
                place_grid(floor, floor.get_grid({ y, x }), GB_INNER);
            }
        }
        ...
        vault_traps(floor, *center, { 4, 10 }, 2 + randint1(3));
    ...
    case 3: {
        ...
        if (one_in_(3)) {
            ...
            for (auto x = center->x - 5; x <= center->x + 5; x++) {
                place_grid(floor, floor.get_grid({ center->y - 1, x }), GB_INNER);
                place_grid(floor, floor.get_grid({ center->y + 1, x }), GB_INNER);
            }

            /* Close off the left/right edges */
            place_grid(floor, floor.get_grid({ center->y, center->x - 5 }), GB_INNER);
            place_grid(floor, floor.get_grid({ center->y, center->x + 5 }), GB_INNER);
    ...
    case 5: {
        ...
        /* Inner "cross" */
        for (auto y = y1_inner; y <= y2_inner; y++) {
            place_grid(floor, floor.get_grid({ y, center->x }), GB_INNER);
        }

        for (auto x = x1_inner; x <= x2_inner; x++) {
            place_grid(floor, floor.get_grid({ center->y, x }), GB_INNER);
        }
