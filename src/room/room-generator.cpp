...
bool generate_rooms(player_type *player_ptr, dun_data_type *dd_ptr)
{
     ...
     */
    if (ironman_rooms && d_info[static_cast<int>(floor_ptr->dungeon_idx)].flags.has_none_of( {DF::BEGINNER, DF::CHAMELEON, DF::SMALLEST })) {
        for (int i = 0; i < ROOM_T_MAX; i++) {
    ...
    } else if (d_info[static_cast<int>(floor_ptr->dungeon_idx)].flags.has(DF::NO_VAULT)) {
    ...
    if (d_info[static_cast<int>(floor_ptr->dungeon_idx)].flags.has(DF::BEGINNER))
    ...
    if (d_info[static_cast<int>(floor_ptr->dungeon_idx)].flags.has(DF::NO_CAVE)) {
    ...
    } else if (d_info[static_cast<int>(floor_ptr->dungeon_idx)].flags.has(DF::CAVE)) {
    ...
    if (d_info[static_cast<int>(floor_ptr->dungeon_idx)].flags.has_not(DF::GLASS_ROOM))
        prob_list[ROOM_T_GLASS] = 0;

    /*! @details ARCADEは同フラグがダンジョンにないと生成されない。 / Forbidden glass rooms */
    if (d_info[static_cast<int>(floor_ptr->dungeon_idx)].flags.has_not(DF::ARCADE))
