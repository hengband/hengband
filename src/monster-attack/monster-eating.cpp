...
static void move_item_to_monster(player_type *player_ptr, monap_type *monap_ptr, const OBJECT_IDX o_idx)
{
    ...
    j_ptr->number = 1;
    if ((monap_ptr->o_ptr->tval == ItemPrimaryType::TV_ROD) || (monap_ptr->o_ptr->tval == ItemPrimaryType::TV_WAND)) {
...
void process_eat_food(player_type *player_ptr, monap_type *monap_ptr)
{
    for (int i = 0; i < 10; i++) {
        ...
        if ((monap_ptr->o_ptr->tval != ItemPrimaryType::TV_FOOD) && !((monap_ptr->o_ptr->tval == ItemPrimaryType::TV_CORPSE) && (monap_ptr->o_ptr->sval)))
            continue;
...
{
    if (((monap_ptr->o_ptr->tval != ItemPrimaryType::TV_STAFF) && (monap_ptr->o_ptr->tval != ItemPrimaryType::TV_WAND)) || (monap_ptr->o_ptr->pval == 0))
        return false;
    ...
    if (monap_ptr->o_ptr->tval == ItemPrimaryType::TV_STAFF)
