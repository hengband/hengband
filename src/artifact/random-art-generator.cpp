...
static bool decide_random_art_cursed(const bool a_scroll, object_type *o_ptr)
{
    ...
    if (((o_ptr->tval == ItemPrimaryType::TV_AMULET) || (o_ptr->tval == ItemPrimaryType::TV_RING)) && o_ptr->is_cursed())
        return true;
...
static void invest_powers(player_type *player_ptr, object_type *o_ptr, int *powers, bool *has_pval, const bool a_cursed)
{
    ...
    while ((*powers)--) {
        ...
        case 3:
        case 4:
            if (one_in_(2) && o_ptr->is_weapon_ammo() && (o_ptr->tval != ItemPrimaryType::TV_BOW)) {
...
static void strengthen_pval(object_type *o_ptr)
{
    if (o_ptr->art_flags.has(TR_BLOWS)) {
        ...
        if ((o_ptr->tval == ItemPrimaryType::TV_SWORD) && (o_ptr->sval == SV_HAYABUSA))
            o_ptr->pval++;
    } else {
        do {
            o_ptr->pval++;
...
{
    if ((o_ptr->tval != ItemPrimaryType::TV_SWORD) || (o_ptr->sval != SV_POISON_NEEDLE))
        return;
...
bool become_random_artifact(player_type *player_ptr, object_type *o_ptr, bool a_scroll)
{
    ...
    if (((o_ptr->artifact_bias == BIAS_MAGE) || (o_ptr->artifact_bias == BIAS_INT)) && (o_ptr->tval == ItemPrimaryType::TV_GLOVES))
