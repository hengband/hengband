...
{
    if ((o_ptr->tval != ItemPrimaryType::TV_GLOVES) || o_ptr->art_flags.has(TR_MAGIC_MASTERY))
        return false;
...
{
    if (((o_ptr->artifact_bias != BIAS_MAGE) && (o_ptr->artifact_bias != BIAS_PRIESTLY)) || (o_ptr->tval != ItemPrimaryType::TV_SOFT_ARMOR) || (o_ptr->sval != SV_ROBE)
