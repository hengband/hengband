...
{
    if (o_ptr->tval != ItemPrimaryType::TV_BOW)
        return false;
...
static bool switch_random_art_slay(object_type *o_ptr)
{
    ...
    case BIAS_PRIESTLY:
        if (((o_ptr->tval == ItemPrimaryType::TV_SWORD) || (o_ptr->tval == ItemPrimaryType::TV_POLEARM)) && o_ptr->art_flags.has_not(TR_BLESSED))
    ...
    case BIAS_ROGUE:
        if ((((o_ptr->tval == ItemPrimaryType::TV_SWORD) && (o_ptr->sval == SV_DAGGER)) || ((o_ptr->tval == ItemPrimaryType::TV_POLEARM) && (o_ptr->sval == SV_SPEAR)))
