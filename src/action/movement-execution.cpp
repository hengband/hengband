...
void exe_movement(PlayerType *player_ptr, DIRECTION dir, bool do_pickup, bool break_trap)
{
    ...
    if (can_move && player_ptr->riding) {
        if (riding_monrace.behavior_flags.has(MonsterBehaviorType::NEVER_MOVE)) {
            ...
            msg_print(_(format("%sの上に行けない。", grid.get_apparent_terrain(TerrainKind::MIMIC).name.data()), "Can't swim."));
            energy.reset_player_turn();
            can_move = false;
            disturb(player_ptr, false, true);
            ...
            msg_format(fmt, floor.get_grid(p_pos).get_apparent_terrain(TerrainKind::MIMIC).name.data());
            energy.reset_player_turn();
            can_move = false;
            disturb(player_ptr, false, true);
        } else if (terrain.flags.has(TerrainCharacteristics::LAVA) && riding_monrace.resistance_flags.has_none_of(RFR_EFF_IM_FIRE_MASK)) {
            msg_print(_(format("%sの上に行けない。", grid.get_apparent_terrain(TerrainKind::MIMIC).name.data()), "Too hot to go through."));
            energy.reset_player_turn();
            can_move = false;
            disturb(player_ptr, false, true);
        }
    ...
    if (!can_move) {
        ...
        msg_format(_("空を飛ばないと%sの上には行けない。", "You need to fly to go through the %s."), grid.get_apparent_terrain(TerrainKind::MIMIC).name.data());
        energy.reset_player_turn();
        ...
        const auto &terrain_mimic = grid.get_apparent_terrain(TerrainKind::MIMIC);
