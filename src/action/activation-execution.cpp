...
static void decide_activation_level(ae_type *ae_ptr)
{
    ...
    if (((ae_ptr->o_ptr->tval == ItemPrimaryType::TV_RING) || (ae_ptr->o_ptr->tval == ItemPrimaryType::TV_AMULET)) && ae_ptr->o_ptr->name2)
...
static bool check_activation_conditions(player_type *player_ptr, ae_type *ae_ptr)
{
    ...
    if (!ae_ptr->o_ptr->xtra4 && (ae_ptr->o_ptr->tval == ItemPrimaryType::TV_FLASK) && ((ae_ptr->o_ptr->sval == SV_LITE_TORCH) || (ae_ptr->o_ptr->sval == SV_LITE_LANTERN))) {
...
static bool activate_artifact(player_type *player_ptr, object_type *o_ptr)
{
    ...
    case RandomArtActType::BR_FIRE:
        o_ptr->timeout = ((o_ptr->tval == ItemPrimaryType::TV_RING) && (o_ptr->sval == SV_RING_FLAMES)) ? 200 : 250;
        return true;
    case RandomArtActType::BR_COLD:
        o_ptr->timeout = ((o_ptr->tval == ItemPrimaryType::TV_RING) && (o_ptr->sval == SV_RING_ICE)) ? 200 : 250;
        return true;
...
{
    if (ae_ptr->o_ptr->tval != ItemPrimaryType::TV_WHISTLE)
        return false;
