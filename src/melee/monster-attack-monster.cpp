...
static void aura_fire_by_melee(PlayerType *player_ptr, mam_type *mam_ptr)
{
    ...
    if (tr_ptr->aura_flags.has_not(MonsterAuraType::FIRE) || !is_valid_monster_race(mam_ptr->m_ptr->r_idx)) {
        return;
    }
...
static void aura_cold_by_melee(PlayerType *player_ptr, mam_type *mam_ptr)
{
    auto *r_ptr = &r_info[mam_ptr->m_ptr->r_idx];
    monster_race *tr_ptr = &r_info[mam_ptr->t_ptr->r_idx];
    if (tr_ptr->aura_flags.has_not(MonsterAuraType::COLD) || !is_valid_monster_race(mam_ptr->m_ptr->r_idx)) {
        return;
    }
...
static void aura_elec_by_melee(PlayerType *player_ptr, mam_type *mam_ptr)
{
    auto *r_ptr = &r_info[mam_ptr->m_ptr->r_idx];
    monster_race *tr_ptr = &r_info[mam_ptr->t_ptr->r_idx];
    if (tr_ptr->aura_flags.has_not(MonsterAuraType::ELEC) || !is_valid_monster_race(mam_ptr->m_ptr->r_idx)) {
        return;
    }
...
bool monst_attack_monst(PlayerType *player_ptr, MONSTER_IDX m_idx, MONSTER_IDX t_idx)
{
    ...
    if (!mam_ptr->blinked || !is_valid_monster_race(mam_ptr->m_ptr->r_idx)) {
        return true;
    }
