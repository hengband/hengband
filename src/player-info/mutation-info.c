...
void set_mutation_info_2(player_type *creature_ptr, self_info_type *self_ptr)
{
    ...
    if (any_bits(creature_ptr->muta2, MUT2_BERS_RAGE))
...
void set_mutation_info_3(player_type *creature_ptr, self_info_type *self_ptr)
{
    ...
    if (any_bits(creature_ptr->muta3, MUT3_ELEC_TOUC))
        self_ptr->info[self_ptr->line++] = _("あなたの血管には電流が流れている。", "Electricity is running through your veins.");

    if (any_bits(creature_ptr->muta3, MUT3_FIRE_BODY))
    ...
    if (any_bits(creature_ptr->muta3, MUT3_WINGS))
    ...
    if (any_bits(creature_ptr->muta3, MUT3_ESP)) {
        /* Unnecessary */
    }
    ...
    if (any_bits(creature_ptr->muta3, MUT3_VULN_ELEM))
    ...
    if (any_bits(creature_ptr->muta3, MUT3_BAD_LUCK))
