 ...
 * * 各要素によるステータス修正値の合計
 */
short PlayerBasicStatistics::modification_value()
{
 ...
 */
short PlayerBasicStatistics::get_value()
{
    this->set_locals();
 ...
 * * 種族によるステータス修正値。
 */
short PlayerBasicStatistics::race_value()
{
 ...
 * * 職業によるステータス修正値
 */
short PlayerBasicStatistics::class_value()
{
 ...
 * * 性格によるステータス修正値
 */
short PlayerBasicStatistics::personality_value()
{
...
void PlayerBasicStatistics::update_top_status()
{
    ...
    if (this->owner_ptr->stat_top[status] != top) {
        this->owner_ptr->stat_top[status] = (short)top;
 ...
 */
short PlayerBasicStatistics::set_exception_use_status(short value)
{
    return value;
}
...
{
    int status = (int)this->ability_type;
    short use = modify_stat_value(this->owner_ptr->stat_cur[status], this->owner_ptr->stat_add[status]);

    use = this->set_exception_use_status(use);

    if (this->owner_ptr->stat_use[status] != use) {
        this->owner_ptr->stat_use[status] = (short)use;
        set_bits(this->owner_ptr->redraw, PR_STATS);
        set_bits(this->owner_ptr->window_flags, PW_PLAYER);
    }
}
...
void PlayerBasicStatistics::update_index_status()
{
    ...
    this->owner_ptr->stat_index[status] = (short)index;
    if (status == A_CON) {
