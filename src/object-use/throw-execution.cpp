...
bool ObjectThrowEntity::check_can_throw()
{
    ...
    if (this->player_ptr->current_floor_ptr->inside_arena && !this->boomerang && (this->o_ptr->tval != ItemPrimaryType::TV_SPIKE)) {
...
void ObjectThrowEntity::set_class_specific_throw_params()
{
    ...
    this->shuriken = (this->player_ptr->pclass == CLASS_NINJA)
        && ((this->q_ptr->tval == ItemPrimaryType::TV_SPIKE) || ((this->obj_flags.has(TR_THROW)) && (this->q_ptr->tval == ItemPrimaryType::TV_SWORD)));
}
...
{
    if ((this->q_ptr->tval != ItemPrimaryType::TV_FIGURINE) || this->player_ptr->current_floor_ptr->inside_arena) {
        return;
    }

    this->corruption_possibility = 100;
    if (!(summon_named_creature(this->player_ptr, 0, this->y, this->x, this->q_ptr->pval, !(this->q_ptr->is_cursed()) ? PM_FORCE_PET : PM_NONE))) {
...
bool ObjectThrowEntity::check_racial_target_bold()
{
    ...
    this->hit_wall = true;
    return (this->q_ptr->tval == ItemPrimaryType::TV_FIGURINE) || this->q_ptr->is_potion()
