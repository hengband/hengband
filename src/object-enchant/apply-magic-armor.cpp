...
void ArmorEnchanter::give_ego_index()
{
    ...
    while (true) {
        ...
        case EGO_DWARVEN:
            if (this->o_ptr->tval != ItemPrimaryType::TV_HARD_ARMOR) {
                valid = false;
            }

            break;
        case EGO_DRUID:
            if (this->o_ptr->tval != ItemPrimaryType::TV_SOFT_ARMOR) {
                valid = false;
            }
...
void ArmorEnchanter::give_high_ego_index()
{
    ...
    this->o_ptr->k_idx = lookup_kind(ItemPrimaryType::TV_SOFT_ARMOR, SV_TWILIGHT_ROBE);
