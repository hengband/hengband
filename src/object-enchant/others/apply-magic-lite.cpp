...
void LiteEnchanter::apply_magic()
{
    if (this->power > 2) {
    ...
    } else if ((this->power == 2) || ((this->power == 1) && one_in_(3))) {
        ...
        case EGO_LITE_DARKNESS:
            ...
            if (this->o_ptr->sval == SV_LITE_TORCH) {
                this->o_ptr->art_flags.set(TR_LITE_M1);
            } else if (this->o_ptr->sval == SV_LITE_LANTERN) {
                this->o_ptr->art_flags.set(TR_LITE_M2);
            } else if (this->o_ptr->sval == SV_LITE_FEANOR) {
