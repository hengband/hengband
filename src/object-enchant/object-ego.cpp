...
static void ego_interpret_extra_abilities(object_type *o_ptr, ego_item_type *e_ptr, EnumClassFlagGroup<TRG> &gen_flags)
{
    for (auto& xtra : e_ptr->xtra_flags) {
        ...
        if (n > 0) {
            ...
            auto except = (f == TR_VORPAL && o_ptr->tval != ItemPrimaryType::TV_SWORD);
            if (!except)
...
void apply_ego(object_type *o_ptr, DEPTH lev)
{
    ...
    if (is_cursed) {
        ...
        if ((o_ptr->tval == ItemPrimaryType::TV_SWORD) && (o_ptr->sval == SV_HAYABUSA) && (o_ptr->pval > 2) && (o_ptr->name2 != EGO_ATTACKS))
            o_ptr->pval = 2;
    }
}
