...
void apply_magic_weapon(player_type *player_ptr, object_type *o_ptr, DEPTH level, int power)
{
    ...
    if ((o_ptr->tval == ItemPrimaryType::TV_SWORD) && (o_ptr->sval == SV_DIAMOND_EDGE))
        return;
    ...
    case ItemPrimaryType::TV_SWORD: {
        if (power > 1) {
            ...
            while (true) {
                ...
                if (o_ptr->name2 == EGO_SHARPNESS && o_ptr->tval != ItemPrimaryType::TV_SWORD)
                    continue;
                if (o_ptr->name2 == EGO_EARTHQUAKES && o_ptr->tval != ItemPrimaryType::TV_HAFTED)
                    continue;
                break;
            }
            ...
            if (randint0(MAX_DEPTH) < level) {
                ...
                while (true) {
                    ...
                    if (o_ptr->name2 == EGO_WEIRD && o_ptr->tval != ItemPrimaryType::TV_SWORD) {
                        continue;
                    }

                    ego_item_type *e_ptr = &e_info[o_ptr->name2];
                    if (o_ptr->tval == ItemPrimaryType::TV_SWORD && o_ptr->sval == SV_HAYABUSA && e_ptr->max_pval < 0) {
                        if (++n > 1000) {
