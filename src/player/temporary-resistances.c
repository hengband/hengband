...
void tim_player_flags(player_type *creature_ptr, BIT_FLAGS *flags)
{
	...
	if (creature_ptr->tim_res_nether) add_flag(flags, TR_RES_NETHER);

	if (creature_ptr->tim_sh_fire) add_flag(flags, TR_SH_FIRE);
	...
	if (hex_spelling(creature_ptr, HEX_ICE_ARMOR)) add_flag(flags, TR_SH_COLD);
	if (hex_spelling(creature_ptr, HEX_SHOCK_CLOAK)) add_flag(flags, TR_SH_ELEC);
}
