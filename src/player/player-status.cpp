...
static void update_num_of_spells(player_type *player_ptr)
{
    ...
    int bonus = 0;
    if ((player_ptr->pclass != CLASS_SAMURAI) && (mp_ptr->spell_book != ItemPrimaryType::TV_LIFE_BOOK)) {
        bonus = 4;
    }
    ...
    if (player_ptr->realm2 == REALM_NONE) {
        ...
        if (k > 32)
            k = 32;
        if ((player_ptr->new_spells > k) && ((mp_ptr->spell_book == ItemPrimaryType::TV_LIFE_BOOK) || (mp_ptr->spell_book == ItemPrimaryType::TV_HISSATSU_BOOK))) {
...
{
    if ((mp_ptr->spell_book == ItemPrimaryType::TV_NONE) && mp_ptr->spell_first == SPELL_FIRST_NO_SPELL)
        return;
...
int16_t calc_num_fire(player_type *player_ptr, object_type *o_ptr)
{
    ...
    ItemPrimaryType tval_ammo = bow_tval_ammo(o_ptr);
    if ((player_ptr->pclass == CLASS_RANGER) && (tval_ammo == ItemPrimaryType::TV_ARROW)) {
        num += (player_ptr->lev * 4);
    }

    if ((player_ptr->pclass == CLASS_CAVALRY) && (tval_ammo == ItemPrimaryType::TV_ARROW)) {
        num += (player_ptr->lev * 3);
    }

    if (player_ptr->pclass == CLASS_ARCHER) {
        if (tval_ammo == ItemPrimaryType::TV_ARROW)
            num += ((player_ptr->lev * 5) + 50);
        else if ((tval_ammo == ItemPrimaryType::TV_BOLT) || (tval_ammo == ItemPrimaryType::TV_SHOT))
            num += (player_ptr->lev * 4);
    }

    if (player_ptr->pclass == CLASS_WARRIOR && (tval_ammo <= ItemPrimaryType::TV_BOLT) && (tval_ammo >= ItemPrimaryType::TV_SHOT)) {
        num += (player_ptr->lev * 2);
    }

    if ((player_ptr->pclass == CLASS_ROGUE) && (tval_ammo == ItemPrimaryType::TV_SHOT)) {
        num += (player_ptr->lev * 4);
    }
...
int16_t calc_double_weapon_penalty(player_type *player_ptr, INVENTORY_IDX slot)
{
    ...
    if (has_melee_weapon(player_ptr, INVEN_MAIN_HAND) && has_melee_weapon(player_ptr, INVEN_SUB_HAND)) {
        ...
        if (player_ptr->inventory_list[slot].tval == ItemPrimaryType::TV_POLEARM)
            penalty += 10;
    }
...
static int16_t calc_riding_bow_penalty(player_type *player_ptr)
{
    ...
    if ((player_ptr->pclass == CLASS_BEASTMASTER) || (player_ptr->pclass == CLASS_CAVALRY)) {
        if (player_ptr->tval_ammo != ItemPrimaryType::TV_ARROW)
            penalty = 5;
    } else {
...
static int16_t calc_to_damage(player_type *player_ptr, INVENTORY_IDX slot, bool is_real_value)
{
    ...
    if ((player_ptr->pclass == CLASS_PRIEST) && (flgs.has_not(TR_BLESSED)) && ((o_ptr->tval == ItemPrimaryType::TV_SWORD) || (o_ptr->tval == ItemPrimaryType::TV_POLEARM))) {
        damage -= 2;
    } else if (player_ptr->pclass == CLASS_BERSERKER) {
        ...
        if (!((o_ptr->tval == ItemPrimaryType::TV_HAFTED) && ((o_ptr->sval == SV_WIZSTAFF) || (o_ptr->sval == SV_NAMAKE_HAMMER)))) {
            damage -= 200;
        } else {
            damage -= 10;
        }
    ...
    for (int i = INVEN_MAIN_HAND; i < INVEN_TOTAL; i++) {
        int bonus_to_d = 0;
        o_ptr = &player_ptr->inventory_list[i];
        if (!o_ptr->k_idx || o_ptr->tval == ItemPrimaryType::TV_CAPTURE || (i == INVEN_MAIN_HAND && has_melee_weapon(player_ptr, i))
