...
void wreck_the_pattern(PlayerType *player_ptr)
{
    ...
    const auto &terrain = floor_ptr->get_grid(p_pos).get_apparent_terrain();
    ...
    while (to_ruin--) {
        ...
        const Pos2D pos(y, x);
        if (pattern_tile(floor_ptr, pos.y, pos.x) && (floor_ptr->get_grid(pos).get_apparent_terrain().subtype != PATTERN_TILE_WRECKED)) {
