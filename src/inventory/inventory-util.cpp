 ...
 * and "x" is the "current" command_cmd code.\n
 */
bool get_tag(player_type *owner_ptr, COMMAND_CODE *cp, char tag, BIT_FLAGS mode, tval_type tval, const ItemTester& item_tester)
{
    ...
    for (COMMAND_CODE i = start; i <= end; i++) {
        ...
        if (!item_tester.okay(owner_ptr, o_ptr, tval) && !(mode & USE_FULL))
            continue;
 ...
 */
bool get_item_okay(player_type *owner_ptr, OBJECT_IDX i, tval_type item_tester_tval, const ItemTester& item_tester)
{
    ...
    return item_tester.okay(owner_ptr, &owner_ptr->inventory_list[i], item_tester_tval);
}
 ...
 */
void prepare_label_string(player_type *owner_ptr, char *label, BIT_FLAGS mode, tval_type tval, const ItemTester& item_tester)
{
    ...
    for (int i = 0; i < 52; i++) {
        ...
        if (!get_tag(owner_ptr, &index, c, mode, tval, item_tester))
            continue;
