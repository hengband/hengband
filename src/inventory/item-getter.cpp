...
static tl::optional<short> check_item_tag_inventory(PlayerType *player_ptr, ItemSelection *item_selection_ptr, short i_idx, char *prev_tag, const ItemTester &item_tester)
{
    ...
    if (*prev_tag && command_cmd) {
        ...
        auto is_item_not_found = !i_idx_opt.has_value();
        if (i_idx_opt) {
            item_selection_ptr->k = *i_idx_opt;
        }

        is_item_not_found |= !get_item_okay(player_ptr, item_selection_ptr->k, item_tester);
        if (item_selection_ptr->k < INVEN_MAIN_HAND) {
            is_item_not_found |= !item_selection_ptr->inven;
        } else {
            is_item_not_found |= !item_selection_ptr->equip;
        }

        if (is_item_not_found) {
            *prev_tag = '\0';
            return tl::nullopt;
        }
