...
static bool check_item_tag_aux(player_type *owner_ptr, item_selection_type *item_selection_ptr, const ItemTester& item_tester)
{
    ...
    if (!item_tester.okay(owner_ptr, o_ptr, item_selection_ptr->tval) && ((item_selection_ptr->mode & USE_FULL) == 0))
        return false;
...
static bool check_item_tag_inventory(player_type *owner_ptr, item_selection_type *item_selection_ptr, char *prev_tag, const ItemTester& item_tester)
{
    ...
    if (*prev_tag && command_cmd) {
        ...
        flag |= !get_tag(owner_ptr, &item_selection_ptr->k, *prev_tag, use_flag, item_selection_ptr->tval, item_tester);
        flag |= !get_item_okay(owner_ptr, item_selection_ptr->k, item_selection_ptr->tval, item_tester);
    ...
    if (!get_item_okay(owner_ptr, *item_selection_ptr->cp, item_selection_ptr->tval, item_tester))
        return false;
...
static void test_inventory(player_type *owner_ptr, item_selection_type *item_selection_ptr, const ItemTester& item_tester)
{
    ...
    for (int j = 0; j < INVEN_PACK; j++)
        if (item_tester.okay(owner_ptr, &owner_ptr->inventory_list[j], item_selection_ptr->tval) || (item_selection_ptr->mode & USE_FULL))
...
static void test_equipment(player_type *owner_ptr, item_selection_type *item_selection_ptr, const ItemTester& item_tester)
{
    ...
    for (int j = INVEN_MAIN_HAND; j < INVEN_TOTAL; j++)
        if (owner_ptr->select_ring_slot ? is_ring_slot(j)
                             : item_tester.okay(owner_ptr, &owner_ptr->inventory_list[j], item_selection_ptr->tval) || (item_selection_ptr->mode & USE_FULL))
 ...
 */
bool get_item(player_type *owner_ptr, OBJECT_IDX *cp, concptr pmt, concptr str, BIT_FLAGS mode, tval_type tval, const ItemTester& item_tester)
{
    static char prev_tag = '\0';
    if (easy_floor || use_menu)
        return get_item_floor(owner_ptr, cp, pmt, str, mode, tval, item_tester);
    ...
    while ((item_selection_ptr->i1 <= item_selection_ptr->i2) && (!get_item_okay(owner_ptr, item_selection_ptr->i1, item_selection_ptr->tval, item_tester)))
        item_selection_ptr->i1++;

    while ((item_selection_ptr->i1 <= item_selection_ptr->i2) && (!get_item_okay(owner_ptr, item_selection_ptr->i2, item_selection_ptr->tval, item_tester)))
    ...
    while ((item_selection_ptr->e1 <= item_selection_ptr->e2) && (!get_item_okay(owner_ptr, item_selection_ptr->e1, item_selection_ptr->tval, item_tester)))
        item_selection_ptr->e1++;

    while ((item_selection_ptr->e1 <= item_selection_ptr->e2) && (!get_item_okay(owner_ptr, item_selection_ptr->e2, item_selection_ptr->tval, item_tester)))
    ...
    if (item_selection_ptr->floor) {
        for (const auto this_o_idx : owner_ptr->current_floor_ptr->grid_array[owner_ptr->y][owner_ptr->x].o_idx_list) {
            ...
            if ((item_tester.okay(owner_ptr, o_ptr, item_selection_ptr->tval) || (item_selection_ptr->mode & USE_FULL)) && (o_ptr->marked & OM_FOUND))
    ...
    while (!item_selection_ptr->done) {
        ...
        if (!command_wrk) {
            if (command_see)
                get_item_label = show_inventory(owner_ptr, item_selection_ptr->menu_line, item_selection_ptr->mode, item_selection_ptr->tval, item_tester);
        } else {
            if (command_see)
                get_item_label = show_equipment(owner_ptr, item_selection_ptr->menu_line, item_selection_ptr->mode, item_selection_ptr->tval, item_tester);
        }
            ...
            case '\n': {
                if (command_wrk == USE_FLOOR) {
                    *item_selection_ptr->cp = -get_item_label;
                    ...
                    if (!get_item_okay(owner_ptr, get_item_label, item_selection_ptr->tval, item_tester)) {
                        bell();
                        break;
                    }
        ...
        case '-': {
            if (item_selection_ptr->allow_floor) {
                for (const auto this_o_idx : owner_ptr->current_floor_ptr->grid_array[owner_ptr->y][owner_ptr->x].o_idx_list) {
                    object_type *o_ptr;
                    o_ptr = &owner_ptr->current_floor_ptr->o_list[this_o_idx];
                    if (!item_tester.okay(owner_ptr, o_ptr, item_selection_ptr->tval) && !(item_selection_ptr->mode & USE_FULL))
                        continue;
        ...
        case '8':
        case '9': {
            if (!get_tag(owner_ptr, &item_selection_ptr->k, item_selection_ptr->which, command_wrk ? USE_EQUIP : USE_INVEN, item_selection_ptr->tval, item_tester)) {
                bell();
                break;
            }
            ...
            if (!get_item_okay(owner_ptr, item_selection_ptr->k, item_selection_ptr->tval, item_tester)) {
                bell();
                break;
            }
