...
void WorldTurnProcessor::process_world()
{
    ...
    int prev_turn_in_today = ((current_world_ptr->game_turn - TURNS_PER_TICK) % a_day + a_day / 4) % a_day;
...
void WorldTurnProcessor::decide_auto_save()
{
    ...
    should_save &= current_world_ptr->game_turn % ((int)autosave_freq * TURNS_PER_TICK) == 0;
    if (should_save) {
