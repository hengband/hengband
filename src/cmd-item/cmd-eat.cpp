...
{
    if (o_ptr->tval != ItemPrimaryType::TV_FOOD)
        return false;
...
bool exe_eat_charge_of_magic_device(player_type *player_ptr, object_type *o_ptr, INVENTORY_IDX item)
{
    ...
    if (PlayerRace(player_ptr).food() == PlayerRaceFood::MANA) {
        ...
        if (o_ptr->tval == ItemPrimaryType::TV_STAFF && (item < 0) && (o_ptr->number > 1)) {
            msg_print(_("まずは杖を拾わなければ。", "You must first pick up the staffs."));
            return true;
        }

        staff = (o_ptr->tval == ItemPrimaryType::TV_STAFF) ? _("杖", "staff") : _("魔法棒", "wand");
...
void exe_eat_food(player_type *player_ptr, INVENTORY_IDX item)
{
    ...
    if (food_type == PlayerRaceFood::CORPSE && (o_ptr->tval == ItemPrimaryType::TV_CORPSE && o_ptr->sval == SV_CORPSE && angband_strchr("pht", r_info[o_ptr->pval].d_char))) {
    ...
    if (PlayerRace(player_ptr).equals(PlayerRaceType::SKELETON)) {
        ...
        if (o_ptr->tval == ItemPrimaryType::TV_FOOD && o_ptr->sval == SV_FOOD_WAYBREAD) {
