 ...
 * @param col 表示行
 */
static void prt_alloc(ItemPrimaryType tval, OBJECT_SUBTYPE_VALUE sval, TERM_LEN row, TERM_LEN col)
{
...
static int is_slot_able_to_be_ego(player_type *player_ptr, object_type *o_ptr)
{
    ...
    if ((o_ptr->tval == ItemPrimaryType::TV_SHOT) || (o_ptr->tval == ItemPrimaryType::TV_ARROW) || (o_ptr->tval == ItemPrimaryType::TV_BOLT))
        return (INVEN_AMMO);
