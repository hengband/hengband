...
static void generate_world(player_type *player_ptr, bool new_game)
{
    ...
    if (player_ptr->pclass != CLASS_SORCERER) {
        ...
        if (player_ptr->ppersonality == PERSONALITY_SEXY)
            s_info[player_ptr->pclass].w_max[enum2i(ItemPrimaryType::TV_HAFTED) - begin][SV_WHIP] = WEAPON_EXP_MASTER;
        if (player_ptr->prace == PlayerRaceType::MERFOLK) {
            s_info[player_ptr->pclass].w_max[enum2i(ItemPrimaryType::TV_POLEARM) - begin][SV_TRIDENT] = WEAPON_EXP_MASTER;
            s_info[player_ptr->pclass].w_max[enum2i(ItemPrimaryType::TV_POLEARM) - begin][SV_TRIFURCATE_SPEAR] = WEAPON_EXP_MASTER;
        }
    }
