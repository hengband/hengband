 ...
 * @return エラーコード
 */
static errr generate_quest_floor(player_type *player_ptr, qg_type *qg_ptr, errr (*process_dungeon_file_pf)(player_type *, concptr, int, int, int, int))
{
    char *zz[33];
    ...
    if (qg_ptr->buf[0] == '%')
        return (*process_dungeon_file_pf)(player_ptr, qg_ptr->buf + 2, qg_ptr->ymin, qg_ptr->xmin, qg_ptr->ymax, qg_ptr->xmax);
...
errr process_dungeon_file(player_type *player_ptr, concptr name, int ymin, int xmin, int ymax, int xmax)
{
    ...
    while (my_fgets(fp, buf, sizeof(buf)) == 0) {
        ...
        err = generate_quest_floor(player_ptr, qg_ptr, process_dungeon_file);
        if (err)
            break;
    }
