 ...
 * @param dam 基本威力
 * @param attribute 効果属性(後々下記の属性クラスに置き換えていく)
 * @param flag 効果フラグ
 */
...
static bool process_bolt_reflection(PlayerType *player_ptr, EffectPlayerType *ep_ptr, project_func project)
{
    ...
    (*project)(player_ptr, 0, 0, pos.y, pos.x, ep_ptr->dam, ep_ptr->attribute, (PROJECT_STOP | PROJECT_KILL | PROJECT_REFLECTABLE), nullptr, tl::nullopt);
    ...
    BIT_FLAGS flag, FallOffHorseEffect &fall_off_horse_effect, project_func project)
{
    ...
    if ((player_ptr->tim_eyeeye || SpellHex(player_ptr).is_spelling_specific(HEX_EYE_FOR_EYE)) && (ep_ptr->get_damage > 0) && !player_ptr->is_dead && ep_ptr->is_monster()) {
        ...
        (*project)(player_ptr, 0, 0, ep_ptr->m_ptr->fy, ep_ptr->m_ptr->fx, ep_ptr->get_damage, AttributeType::MISSILE, PROJECT_KILL, nullptr, tl::nullopt);
