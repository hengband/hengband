...
bool affect_feature(PlayerType *player_ptr, MONSTER_IDX src_idx, POSITION r, POSITION y, POSITION x, int dam, AttributeType typ)
{
    ...
    case AttributeType::KILL_WALL: {
        ...
        if (known && grid.is_mark()) {
            msg_format(_("%sが溶けて泥になった！", "The %s turns into mud!"), grid.get_apparent_terrain(TerrainKind::MIMIC).name.data());
            obvious = true;
        }
    ...
    case AttributeType::ABYSS: {
        ...
        if (floor.is_in_underground() || !AngbandWorld::get_instance().is_daytime()) {
            for (int j = 0; j < 9; j++) {
                ...
                if (grid_neighbor.get_apparent_terrain(TerrainKind::MIMIC).flags.has(TerrainCharacteristics::GLOW)) {
                    do_dark = false;
                    break;
                }
            }
    ...
    case AttributeType::ROCKET: {
        ...
        if (known && (grid.is_mark())) {
            msg_format(_("%sが割れた！", "The %s crumbled!"), grid.get_apparent_terrain(TerrainKind::MIMIC).name.data());
            sound(SOUND_GLASS);
        }
