...
{
    if ((em_ptr->r_ptr->flags2 & RF2_EMPTY_MIND) == 0)
        return FALSE;
...
{
    bool has_resistance = ((em_ptr->r_ptr->flags2 & (RF2_STUPID | RF2_WEIRD_MIND)) != 0) || ((em_ptr->r_ptr->flags3 & RF3_ANIMAL) != 0)
...
static void effect_monster_psi_resist(player_type *caster_ptr, effect_monster_type *em_ptr)
{
    if (effect_monster_psi_empty_mind(caster_ptr, em_ptr))
        return;
    if (effect_monster_psi_weird_mind(em_ptr))
        return;
...
static void effect_monster_psi_drain_resist(player_type *caster_ptr, effect_monster_type *em_ptr)
{
    em_ptr->note = _("には耐性がある！", " resists!");
    em_ptr->dam /= 3;
    if (effect_monster_psi_drain_corrupted(caster_ptr, em_ptr))
        return;
