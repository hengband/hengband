...
static void decide_initial_items(player_type *player_ptr, object_type *q_ptr)
{
    ...
    case PlayerRaceType::BALROG:
        ...
        for (int i = rand_range(3, 4); i > 0; i--) {
            q_ptr->prep(lookup_kind(ItemPrimaryType::TV_CORPSE, SV_CORPSE));
    ...
    case PlayerRaceType::SPECTRE:
        /* Staff (of Nothing) */
        q_ptr->prep(lookup_kind(ItemPrimaryType::TV_STAFF, SV_STAFF_NOTHING));
        q_ptr->number = 1;
        add_outfit(player_ptr, q_ptr);
        break;
    case PlayerRaceType::ENT:
        /* Potions of Water */
        q_ptr->prep(lookup_kind(ItemPrimaryType::TV_POTION, SV_POTION_WATER));
        q_ptr->number = (ITEM_NUMBER)rand_range(15, 23);
        add_outfit(player_ptr, q_ptr);
        break;
    case PlayerRaceType::ANDROID:
        /* Flasks of oil */
        q_ptr->prep(lookup_kind(ItemPrimaryType::TV_FLASK, SV_ANY));
...
void player_outfit(player_type *player_ptr)
{
    ...
    if ((player_ptr->prace == PlayerRaceType::VAMPIRE) && (player_ptr->pclass != CLASS_NINJA)) {
        q_ptr->prep(lookup_kind(ItemPrimaryType::TV_SCROLL, SV_SCROLL_DARKNESS));
        q_ptr->number = (ITEM_NUMBER)rand_range(2, 5);
        add_outfit(player_ptr, q_ptr);
    } else if (player_ptr->pclass != CLASS_NINJA) {
        q_ptr->prep(lookup_kind(ItemPrimaryType::TV_LITE, SV_LITE_TORCH));
        q_ptr->number = (ITEM_NUMBER)rand_range(3, 7);
    ...
    if (player_ptr->prace == PlayerRaceType::MERFOLK) {
        q_ptr->prep(lookup_kind(ItemPrimaryType::TV_RING, SV_RING_LEVITATION_FALL));
        q_ptr->number = 1;
        add_outfit(player_ptr, q_ptr);
    }

    if ((player_ptr->pclass == CLASS_RANGER) || (player_ptr->pclass == CLASS_CAVALRY)) {
        q_ptr->prep(lookup_kind(ItemPrimaryType::TV_ARROW, SV_AMMO_NORMAL));
        q_ptr->number = (byte)rand_range(15, 20);
        add_outfit(player_ptr, q_ptr);
    }

    if (player_ptr->pclass == CLASS_RANGER) {
        q_ptr->prep(lookup_kind(ItemPrimaryType::TV_BOW, SV_SHORT_BOW));
        add_outfit(player_ptr, q_ptr);
        ...
        q_ptr->prep(lookup_kind(ItemPrimaryType::TV_WAND, SV_WAND_MAGIC_MISSILE));
        q_ptr->number = 1;
        q_ptr->pval = (PARAMETER_VALUE)rand_range(25, 30);
        add_outfit(player_ptr, q_ptr);
    } else if (player_ptr->pclass == CLASS_SORCERER) {
        for (auto book_tval = enum2i(ItemPrimaryType::TV_LIFE_BOOK); book_tval <= enum2i(ItemPrimaryType::TV_LIFE_BOOK) + MAX_MAGIC - 1; book_tval++) {
            q_ptr->prep(lookup_kind(i2enum<ItemPrimaryType>(book_tval), 0));
            q_ptr->number = 1;
            add_outfit(player_ptr, q_ptr);
        }
    } else if (player_ptr->pclass == CLASS_TOURIST) {
        if (player_ptr->ppersonality != PERSONALITY_SEXY) {
            q_ptr->prep(lookup_kind(ItemPrimaryType::TV_SHOT, SV_AMMO_LIGHT));
            q_ptr->number = rand_range(15, 20);
            add_outfit(player_ptr, q_ptr);
        }

        q_ptr->prep(lookup_kind(ItemPrimaryType::TV_FOOD, SV_FOOD_BISCUIT));
        q_ptr->number = rand_range(2, 4);

        add_outfit(player_ptr, q_ptr);

        q_ptr->prep(lookup_kind(ItemPrimaryType::TV_FOOD, SV_FOOD_WAYBREAD));
        q_ptr->number = rand_range(2, 4);

        add_outfit(player_ptr, q_ptr);

        q_ptr->prep(lookup_kind(ItemPrimaryType::TV_FOOD, SV_FOOD_JERKY));
        q_ptr->number = rand_range(1, 3);

        add_outfit(player_ptr, q_ptr);

        q_ptr->prep(lookup_kind(ItemPrimaryType::TV_FOOD, SV_FOOD_PINT_OF_ALE));
        q_ptr->number = rand_range(2, 4);

        add_outfit(player_ptr, q_ptr);

        q_ptr->prep(lookup_kind(ItemPrimaryType::TV_FOOD, SV_FOOD_PINT_OF_WINE));
        q_ptr->number = rand_range(2, 4);

        add_outfit(player_ptr, q_ptr);
    } else if (player_ptr->pclass == CLASS_NINJA) {
        q_ptr->prep(lookup_kind(ItemPrimaryType::TV_SPIKE, 0));
        q_ptr->number = rand_range(15, 20);
        add_outfit(player_ptr, q_ptr);
    } else if (player_ptr->pclass == CLASS_SNIPER) {
        q_ptr->prep(lookup_kind(ItemPrimaryType::TV_BOLT, SV_AMMO_NORMAL));
        q_ptr->number = rand_range(15, 20);
        add_outfit(player_ptr, q_ptr);
    }
    ...
    if (player_ptr->pclass != CLASS_SORCERER) {
        if (player_ptr->ppersonality == PERSONALITY_SEXY) {
            player_init[player_ptr->pclass][2] = std::make_tuple(ItemPrimaryType::TV_HAFTED, SV_WHIP);            
        } else if (player_ptr->prace == PlayerRaceType::MERFOLK) {
            player_init[player_ptr->pclass][2] = std::make_tuple(ItemPrimaryType::TV_POLEARM, SV_TRIDENT);
        }
    }

    for (auto i = 0; i < 3; i++) {
        auto &[tv, sv] = player_init[player_ptr->pclass][i];
        if ((player_ptr->prace == PlayerRaceType::ANDROID) && ((tv == ItemPrimaryType::TV_SOFT_ARMOR) || (tv == ItemPrimaryType::TV_HARD_ARMOR)))
            continue;

        if (tv == ItemPrimaryType::TV_SORCERY_BOOK)
            tv = i2enum<ItemPrimaryType>(enum2i(ItemPrimaryType::TV_LIFE_BOOK) + player_ptr->realm1 - 1);
        else if (tv == ItemPrimaryType::TV_DEATH_BOOK)
            tv = i2enum<ItemPrimaryType>(enum2i(ItemPrimaryType::TV_LIFE_BOOK) + player_ptr->realm2 - 1);
        else if (tv == ItemPrimaryType::TV_RING && sv == SV_RING_RES_FEAR && player_ptr->prace == PlayerRaceType::BARBARIAN)
            sv = SV_RING_SUSTAIN_STR;
        else if (tv == ItemPrimaryType::TV_RING && sv == SV_RING_SUSTAIN_INT && player_ptr->prace == PlayerRaceType::MIND_FLAYER) {
            tv = ItemPrimaryType::TV_POTION;
            sv = SV_POTION_RESTORE_MANA;
        }

        q_ptr = &forge;
        q_ptr->prep(lookup_kind(tv, sv));
        if ((tv == ItemPrimaryType::TV_SWORD || tv == ItemPrimaryType::TV_HAFTED)
    ...
    k_info[lookup_kind(ItemPrimaryType::TV_POTION, SV_POTION_WATER)].aware = true;
}
