...
static byte count_realm_selection(const uint choices, int *count)
{
...
static void impose_first_realm(const player_type *creature_ptr, uint *choices)
{
...
static void analyze_realms(const player_type *creature_ptr, const uint choices, birth_realm_type *birth_realm_ptr)
{
    for (int i = 0; i < 32; i++) {
        if ((choices & (1UL << i)) == 0)
            continue;
 ...
 */
static byte select_realm(player_type *creature_ptr, uint choices, int *count)
{
