 ...
 * @param prob 元の確率テーブル
 */
static void set_floor_and_wall_aux(short feat_type[100], feat_prob prob[DUNGEON_FEAT_PROB_NUM])
{
...
static short terrain_table[MAX_WILDERNESS][MAX_FEAT_IN_TERRAIN];
...
static void generate_wilderness_area(floor_type *floor_ptr, int terrain, uint seed, bool corner)
{
    ...
    floor_ptr->grid_array[1][1].feat = (short)randint0(table_size);
    floor_ptr->grid_array[MAX_HGT - 2][1].feat = (short)randint0(table_size);
    floor_ptr->grid_array[1][MAX_WID - 2].feat = (short)randint0(table_size);
    floor_ptr->grid_array[MAX_HGT - 2][MAX_WID - 2].feat = (short)randint0(table_size);
    if (corner) {
    ...
    short north_west = floor_ptr->grid_array[1][1].feat;
    short south_west = floor_ptr->grid_array[MAX_HGT - 2][1].feat;
    short north_east = floor_ptr->grid_array[1][MAX_WID - 2].feat;
    short south_east = floor_ptr->grid_array[MAX_HGT - 2][MAX_WID - 2].feat;
...
static short conv_terrain2feat[MAX_WILDERNESS];
...
void wilderness_gen_small(player_type *creature_ptr)
{
    ...
    for (int i = 0; i < current_world_ptr->max_wild_x; i++) {
        for (int j = 0; j < current_world_ptr->max_wild_y; j++) {
            if (wilderness[j][i].town && (wilderness[j][i].town != NO_TOWN)) {
                floor_ptr->grid_array[j][i].feat = (short)feat_town;
                floor_ptr->grid_array[j][i].special = (short)wilderness[j][i].town;
    ...
    floor_ptr->height = (short)current_world_ptr->max_wild_y;
    floor_ptr->width = (short)current_world_ptr->max_wild_x;
...
parse_error_type parse_line_wilderness(player_type *creature_ptr, char *buf, int xmin, int xmax, int *y, int *x)
{
...
#endif
    {
        if ((num = tokenize(buf + 4, 6, zz, 0)) > 1) {
            ...
            if (num > 2)
                w_letter[index].level = (short)atoi(zz[2]);
            else
                w_letter[index].level = 0;

            if (num > 3)
                w_letter[index].town = static_cast<short>(atoi(zz[3]));
            else
 ...
 */
static void init_terrain_table(int terrain, short feat_global, concptr fmt, ...)
{
    va_list vp;
    va_start(vp, fmt);
    ...
    for (concptr p = fmt; *p; p++) {
        ...
        FEAT_IDX feat = (short)va_arg(vp, int);
