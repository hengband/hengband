...
static bool level_gen(player_type *player_ptr, concptr *why)
{
    ...
    if ((always_small_levels || ironman_small_levels || (one_in_(SMALL_LEVEL) && small_levels) || d_info[static_cast<int>(d_idx)].flags.has(DF::BEGINNER)
            || d_info[static_cast<int>(d_idx)].flags.has(DF::SMALLEST))
        && d_info[static_cast<int>(d_idx)].flags.has_not(DF::BIG)) {
        int level_height;
        int level_width;
        if (d_info[static_cast<int>(d_idx)].flags.has(DF::SMALLEST)) {
            level_height = 1;
            level_width = 1;
        } else if (d_info[static_cast<int>(d_idx)].flags.has(DF::BEGINNER)) {
            level_height = 2;
            level_width = 2;
        } else {
