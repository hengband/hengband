...
static byte get_dungeon_feeling(player_type *player_ptr)
{
    ...
    for (MONSTER_IDX i = 1; i < floor_ptr->o_max; i++) {
        ...
        if (o_ptr->tval == ItemPrimaryType::TV_SHIELD && o_ptr->sval == SV_DRAGON_SHIELD)
            delta += 5 * base;

        if (o_ptr->tval == ItemPrimaryType::TV_GLOVES && o_ptr->sval == SV_SET_OF_DRAGON_GLOVES)
            delta += 5 * base;

        if (o_ptr->tval == ItemPrimaryType::TV_BOOTS && o_ptr->sval == SV_PAIR_OF_DRAGON_GREAVE)
            delta += 5 * base;

        if (o_ptr->tval == ItemPrimaryType::TV_HELM && o_ptr->sval == SV_DRAGON_HELM)
            delta += 5 * base;

        if (o_ptr->tval == ItemPrimaryType::TV_RING && o_ptr->sval == SV_RING_SPEED && !o_ptr->is_cursed())
            delta += 25 * base;

        if (o_ptr->tval == ItemPrimaryType::TV_RING && o_ptr->sval == SV_RING_LORDLY && !o_ptr->is_cursed())
            delta += 15 * base;

        if (o_ptr->tval == ItemPrimaryType::TV_AMULET && o_ptr->sval == SV_AMULET_THE_MAGI && !o_ptr->is_cursed())
            delta += 15 * base;
