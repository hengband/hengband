 ...
 */
ITEM_NUMBER scan_floor_items(player_type *owner_ptr, OBJECT_IDX *items, POSITION y, POSITION x, BIT_FLAGS mode, tval_type item_tester_tval, const ItemTester& item_tester)
{
    ...
    for (const auto this_o_idx : floor_ptr->grid_array[y][x].o_idx_list) {
        ...
        if ((mode & SCAN_FLOOR_ITEM_TESTER) && !item_tester.okay(owner_ptr, o_ptr, item_tester_tval))
            continue;
 ...
 * @details
 */
COMMAND_CODE show_floor_items(player_type *owner_ptr, int target_item, POSITION y, POSITION x, TERM_LEN *min_width, tval_type item_tester_tval, const ItemTester& item_tester)
{
    COMMAND_CODE i, m;
    int j, k, l;
    object_type *o_ptr;
    ...
    floor_num = scan_floor_items(owner_ptr, floor_list, y, x, SCAN_FLOOR_ITEM_TESTER | SCAN_FLOOR_ONLY_MARKED, item_tester_tval, item_tester);
    floor_type *floor_ptr = owner_ptr->current_floor_ptr;
