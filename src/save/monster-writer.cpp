...
static void write_monster_info(monster_type *m_ptr, const BIT_FLAGS flags)
{
    ...
    if (flags & SAVE_MON_TARGET_Y)
        wr_s16b((short)m_ptr->target_y);

    if (flags & SAVE_MON_TARGET_X)
        wr_s16b((short)m_ptr->target_x);
...
void wr_monster(monster_type *m_ptr)
{
    ...
    wr_s16b((short)m_ptr->hp);
    wr_s16b((short)m_ptr->maxhp);
    wr_s16b((short)m_ptr->max_maxhp);
...
void wr_lore(MONRACE_IDX r_idx)
{
    ...
    wr_s16b((short)r_ptr->r_sights);
    wr_s16b((short)r_ptr->r_deaths);
    wr_s16b((short)r_ptr->r_pkills);
    wr_s16b((short)r_ptr->r_akills);
    wr_s16b((short)r_ptr->r_tkills);
