...
static void write_monster_flags(monster_type *m_ptr, BIT_FLAGS *flags)
{
    if (!is_original_ap(m_ptr))
        set_bits(*flags, SaveDataMonsterFlagType::SAVE_MON_AP_R_IDX);

    if (m_ptr->sub_align)
        set_bits(*flags, SaveDataMonsterFlagType::SAVE_MON_SUB_ALIGN);

    if (monster_csleep_remaining(m_ptr))
        set_bits(*flags, SaveDataMonsterFlagType::SAVE_MON_CSLEEP);

    if (monster_fast_remaining(m_ptr))
        set_bits(*flags, SaveDataMonsterFlagType::SAVE_MON_FAST);

    if (monster_slow_remaining(m_ptr))
        set_bits(*flags, SaveDataMonsterFlagType::SAVE_MON_SLOW);

    if (monster_stunned_remaining(m_ptr))
        set_bits(*flags, SaveDataMonsterFlagType::SAVE_MON_STUNNED);

    if (monster_confused_remaining(m_ptr))
        set_bits(*flags, SaveDataMonsterFlagType::SAVE_MON_CONFUSED);

    if (monster_fear_remaining(m_ptr))
        set_bits(*flags, SaveDataMonsterFlagType::SAVE_MON_MONFEAR);

    if (m_ptr->target_y)
        set_bits(*flags, SaveDataMonsterFlagType::SAVE_MON_TARGET_Y);

    if (m_ptr->target_x)
        set_bits(*flags, SaveDataMonsterFlagType::SAVE_MON_TARGET_X);

    if (monster_invulner_remaining(m_ptr))
        set_bits(*flags, SaveDataMonsterFlagType::SAVE_MON_INVULNER);

    if (m_ptr->smart.any())
        set_bits(*flags, SaveDataMonsterFlagType::SAVE_MON_SMART);

    if (m_ptr->exp)
        set_bits(*flags, SaveDataMonsterFlagType::SAVE_MON_EXP);

    if (m_ptr->mflag2.any())
        set_bits(*flags, SaveDataMonsterFlagType::SAVE_MON_MFLAG2);

    if (m_ptr->nickname)
        set_bits(*flags, SaveDataMonsterFlagType::SAVE_MON_NICKNAME);

    if (m_ptr->parent_m_idx)
        set_bits(*flags, SaveDataMonsterFlagType::SAVE_MON_PARENT);
