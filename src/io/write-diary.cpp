...
static QUEST_IDX write_floor(player_type *creature_ptr, concptr *note_level, char *note_level_buf)
{
	...
	{
#ifdef JP
		sprintf(note_level_buf, "%d階(%s):", (int)floor_ptr->dun_level, d_info[static_cast<int>(creature_ptr->dungeon_idx)].name.c_str());
#else
		sprintf(note_level_buf, "%s L%d:", d_info[static_cast<int>(creature_ptr->dungeon_idx)].name.c_str(), (int)floor_ptr->dun_level);
#endif
...
errr exe_write_diary(player_type *creature_ptr, int type, int num, concptr note)
{
	...
	case DIARY_MAXDEAPTH:
	{
		fprintf(fff, _(" %2d:%02d %20s %sの最深階%d階に到達した。\n",
			...
			_(d_info[static_cast<int>(creature_ptr->dungeon_idx)].name.c_str(), num),
			_(num, d_info[static_cast<int>(creature_ptr->dungeon_idx)].name.c_str()));
		break;
	}
	case DIARY_TRUMP:
	{
		fprintf(fff, _(" %2d:%02d %20s %s%sの最深階を%d階にセットした。\n",
			...
			_(d_info[static_cast<int>(num)].name.c_str(), (int)max_dlv[static_cast<int>(num)]),
			_((int)max_dlv[static_cast<int>(num)], d_info[static_cast<int>(num)].name.c_str()));
		break;
	}
	case DIARY_STAIR:
	{
	...
	case DIARY_RECALL:
	{
		if (!num)
			fprintf(fff, _(" %2d:%02d %20s 帰還を使って%sの%d階へ下りた。\n", " %2d:%02d %20s recalled to dungeon level %d of %s.\n"),
				hour, min, note_level, _(d_info[static_cast<int>(creature_ptr->dungeon_idx)].name.c_str(), (int)max_dlv[static_cast<int>(creature_ptr->dungeon_idx)]),
				_((int)max_dlv[static_cast<int>(creature_ptr->dungeon_idx)], d_info[static_cast<int>(creature_ptr->dungeon_idx)].name.c_str()));
		else
	...
	case DIARY_WIZ_TELE:
	{
        concptr to = !is_in_dungeon(creature_ptr)
			? _("地上", "the surface")
			: format(_("%d階(%s)", "level %d of %s"), creature_ptr->current_floor_ptr->dun_level, d_info[static_cast<int>(creature_ptr->dungeon_idx)].name.c_str());
