 ...
 * @details
 */
static errr counts_seek(player_type *creature_ptr, int fd, uint where, bool flag)
{
    ...
    int seekpoint = 0;
    uint zero_header[3] = { 0L, 0L, 0L };
    while (true) {
        if (fd_seek(fd, seekpoint + 3 * sizeof(uint)))
            return 1;
        if (fd_read(fd, (char *)(temp2), sizeof(temp2))) {
            ...
            fd_write(fd, (char *)zero_header, 3 * sizeof(uint));
        ...
        seekpoint += 128 + 3 * sizeof(uint);
    }

    return fd_seek(fd, seekpoint + where * sizeof(uint));
}
 ...
 * @details
 */
uint counts_read(player_type *creature_ptr, int where)
{
    char buf[1024];
    ...
    uint count = 0;
    if (counts_seek(creature_ptr, fd, where, false) || fd_read(fd, (char *)(&count), sizeof(uint)))
        count = 0;
 ...
 * @details
 */
errr counts_write(player_type *creature_ptr, int where, uint count)
{
    char buf[1024];
    path_build(buf, sizeof(buf), ANGBAND_DIR_DATA, _("z_info_j.raw", "z_info.raw"));
    ...
    fd_write(fd, (char *)(&count), sizeof(uint));
    safe_setuid_grab(creature_ptr);
