...
void calc_android_exp(player_type *player_ptr)
{
    ...
    for (int i = INVEN_MAIN_HAND; i < INVEN_TOTAL; i++) {
        ...
        if (value > 5000000L)
            value = 5000000L;
        if ((o_ptr->tval == ItemPrimaryType::TV_DRAG_ARMOR) || (o_ptr->tval == ItemPrimaryType::TV_CARD))
            level /= 2;

        if (o_ptr->is_artifact() || o_ptr->is_ego() || (o_ptr->tval == ItemPrimaryType::TV_DRAG_ARMOR) || ((o_ptr->tval == ItemPrimaryType::TV_HELM) && (o_ptr->sval == SV_DRAGON_HELM))
            || ((o_ptr->tval == ItemPrimaryType::TV_SHIELD) && (o_ptr->sval == SV_DRAGON_SHIELD)) || ((o_ptr->tval == ItemPrimaryType::TV_GLOVES) && (o_ptr->sval == SV_SET_OF_DRAGON_GLOVES))
            || ((o_ptr->tval == ItemPrimaryType::TV_BOOTS) && (o_ptr->sval == SV_PAIR_OF_DRAGON_GREAVE)) || ((o_ptr->tval == ItemPrimaryType::TV_SWORD) && (o_ptr->sval == SV_DIAMOND_EDGE))) {
            if (level > 65)
                level = 35 + (level - 65) / 5;
            else if (level > 35)
                level = 25 + (level - 35) / 3;
