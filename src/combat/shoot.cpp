...
void exe_fire(player_type *shooter_ptr, INVENTORY_IDX item, object_type *j_ptr, SPELL_IDX snipe_type)
{
    ...
    ushort path_g[512]; /* For calcuration of path length */
            ...
            if (shooter_ptr->current_floor_ptr->grid_array[y][x].m_idx) {
                ...
                if (test_hit_fire(shooter_ptr, chance - cur_dis, m_ptr, m_ptr->ml, o_name)) {
                    ...
                    if (snipe_type == SP_EXPLODE) {
                        ushort flg = (PROJECT_STOP | PROJECT_JUMP | PROJECT_KILL | PROJECT_GRID);
 ...
 * @return ダメージ期待値
 */
HIT_POINT calc_expect_crit(player_type *shooter_ptr, WEIGHT weight, int plus, HIT_POINT dam, short meichuu, bool dokubari, bool impact)
{
    if (dokubari)
        return dam;
