...
void do_cmd_knowledge_virtues(PlayerType *player_ptr)
{
    ...
    angband_fclose(fff);
    (void)show_file(player_ptr->name, true, file_name, 0, 0, _("八つの徳", "Virtues"));
    fd_kill(file_name);
}
...
void do_cmd_knowledge_stat(PlayerType *player_ptr)
{
    ...
    (void)show_file(player_ptr->name, true, file_name, 0, 0, _("自分に関する情報", "HP-rate & Max stat"));
    fd_kill(file_name);
}
...
void do_cmd_knowledge_home(PlayerType *player_ptr)
{
    ...
    if (store.stock_num == 0) {
        angband_fclose(fff);
        (void)show_file(player_ptr->name, true, file_name, 0, 0, home_inventory);
        fd_kill(file_name);
        return;
    }
