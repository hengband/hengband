 ...
 */
static bool determine_spcial_item_type(object_type *o_ptr, ItemPrimaryType tval)
{
    bool is_special_item_type = (o_ptr->is_wearable() && o_ptr->is_ego()) || ((tval == ItemPrimaryType::TV_AMULET) && (o_ptr->sval == SV_AMULET_RESISTANCE))
        || ((tval == ItemPrimaryType::TV_RING) && (o_ptr->sval == SV_RING_LORDLY)) || ((tval == ItemPrimaryType::TV_SHIELD) && (o_ptr->sval == SV_DRAGON_SHIELD))
        || ((tval == ItemPrimaryType::TV_HELM) && (o_ptr->sval == SV_DRAGON_HELM)) || ((tval == ItemPrimaryType::TV_GLOVES) && (o_ptr->sval == SV_SET_OF_DRAGON_GLOVES))
        || ((tval == ItemPrimaryType::TV_BOOTS) && (o_ptr->sval == SV_PAIR_OF_DRAGON_GREAVE)) || o_ptr->is_artifact();
 ...
 * @return 必要があるならTRUE
 */
static bool check_item_knowledge(object_type *o_ptr, ItemPrimaryType tval)
{
    if (o_ptr->k_idx == 0)
        return false;
    if (o_ptr->tval != tval)
 ...
 */
static void show_wearing_equipment_resistances(player_type *player_ptr, ItemPrimaryType tval, int *label_number, FILE *fff)
{
    char where[32];
    strcpy(where, _("装", "E "));
 ...
 * @param fff ファイルへの参照ポインタ
 */
static void show_holding_equipment_resistances(player_type *player_ptr, ItemPrimaryType tval, int *label_number, FILE *fff)
{
    char where[32];
    strcpy(where, _("持", "I "));
 ...
 * @param fff ファイルへの参照ポインタ
 */
static void show_home_equipment_resistances(player_type *player_ptr, ItemPrimaryType tval, int *label_number, FILE *fff)
{
...
void do_cmd_knowledge_inventory(player_type *player_ptr)
{
    ...
    for (auto tval = enum2i(TV_WEARABLE_BEGIN); tval <= enum2i(TV_WEARABLE_END); tval++) {
        ...
        show_wearing_equipment_resistances(player_ptr, i2enum<ItemPrimaryType>(tval), &label_number, fff);
        show_holding_equipment_resistances(player_ptr, i2enum<ItemPrimaryType>(tval), &label_number, fff);
        show_home_equipment_resistances(player_ptr, i2enum<ItemPrimaryType>(tval), &label_number, fff);
    }
