...
void do_cmd_knowledge_pets(PlayerType *player_ptr)
{
    ...
    angband_fclose(fff);
    (void)show_file(player_ptr->name, true, file_name, 0, 0, _("現在のペット", "Current Pets"));
    fd_kill(file_name);
}
...
void do_cmd_knowledge_kill_count(PlayerType *player_ptr)
{
    ...
    angband_fclose(fff);
    (void)show_file(player_ptr->name, true, file_name, 0, 0, _("倒した敵の数", "Kill Count"));
    fd_kill(file_name);
}
...
void do_cmd_knowledge_bounty(PlayerType *player_ptr)
{
    ...
    angband_fclose(fff);
    (void)show_file(player_ptr->name, true, file_name, 0, 0, _("賞金首の一覧", "Wanted monsters"));
    fd_kill(file_name);
}
