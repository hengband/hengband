...
display_flag_aux(row+4, col, "テレパシー:", TR_TELEPATHY, &f, DP_ESP);
...
static void display_player_other_flag_info(void)
{
	...
	c_put_str(TERM_WHITE, "abcdefghijkl@", row-1, col+14);
        ...
        display_flag_aux(row+ 9, col, "反テレポート:", TR_NO_TELE, &f, 0);
        display_flag_aux(row+10, col, "反魔法      :", TR_NO_MAGIC, &f, 0);
        display_flag_aux(row+11, col, "消費魔力減少:", TR_DEC_MANA, &f, 0);
        display_flag_aux(row+12, col, "警告        :", TR_WARNING, &f, 0);
        display_flag_aux(row+13, col, "祝福        :", TR_BLESSED, &f, 0);
        display_flag_aux(row+15, col, "経験値減少  :", TR_DRAIN_EXP, &f, 0);
