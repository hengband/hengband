...
std::optional<std::string> do_hissatsu_spell(PlayerType *player_ptr, SPELL_IDX spell_id, SpellProcessType mode)
{
    ...
    case 7:
        if (cast) {
            ...
            if (!player_can_enter(player_ptr, grid_target.feat, 0) || is_trap(grid_target.feat)) {
                break;
            }

            const Pos2D pos_opposite(pos_target.y + ddy[*dir], pos_target.x + ddx[*dir]);
            const auto &grid_opposite = floor.get_grid(pos_opposite);
            if (player_can_enter(player_ptr, grid_opposite.feat, 0) && !is_trap(grid_opposite.feat) && !grid_opposite.m_idx) {
                msg_print(nullptr);
