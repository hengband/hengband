...
static const short CONTINUOUS_DESCRIPTION = 256;
...
static void evaluate_monster_exp(player_type *creature_ptr, char *buf, monster_type *m_ptr)
{
    ...
    int exp_mon = ap_r_ptr->mexp * ap_r_ptr->level;
    uint exp_mon_frac = 0;
    s64b_div(&exp_mon, &exp_mon_frac, 0, (creature_ptr->max_plv + 2));

    int exp_adv = player_exp[creature_ptr->lev - 1] * creature_ptr->expfact;
    uint exp_adv_frac = 0;
...
static bool within_char_util(short input)
{
    return (input > -127) && (input < 128);
}

static short describe_grid(player_type *subject_ptr, eg_type *eg_ptr)
{
...
static short describe_footing(player_type *subject_ptr, eg_type *eg_ptr)
{
...
static short describe_footing_items(eg_type *eg_ptr)
{
    if (!eg_ptr->boring)
        return CONTINUOUS_DESCRIPTION;
...
static short loop_describing_grid(player_type *subject_ptr, eg_type *eg_ptr)
{
    if (eg_ptr->floor_num == 0)
        return CONTINUOUS_DESCRIPTION;
    ...
    int min_width = 0;
    while (true) {
        short footing_description = describe_footing(subject_ptr, eg_ptr);
        if (within_char_util(footing_description))
            return (char)footing_description;

        short footing_descriptions = describe_footing_items(eg_ptr);
...
static short describe_footing_sight(player_type *subject_ptr, eg_type *eg_ptr, object_type *o_ptr)
{
...
static short sweep_footing_items(player_type *subject_ptr, eg_type *eg_ptr)
{
    for (const auto this_o_idx : eg_ptr->g_ptr->o_idx_list) {
        object_type *o_ptr;
        o_ptr = &subject_ptr->current_floor_ptr->o_list[this_o_idx];
        short ret = describe_footing_sight(subject_ptr, eg_ptr, o_ptr);
...
char examine_grid(player_type *subject_ptr, const POSITION y, const POSITION x, target_type mode, concptr info)
{
    ...
    short description_grid = describe_grid(subject_ptr, eg_ptr);
    if (within_char_util(description_grid))
        return (char)description_grid;

    short loop_description = loop_describing_grid(subject_ptr, eg_ptr);
    if (within_char_util(loop_description))
        return (char)loop_description;

    short footing_items_description = sweep_footing_items(subject_ptr, eg_ptr);
