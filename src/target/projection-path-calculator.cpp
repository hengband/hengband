 ...
 * return 経路座標
 */
static ushort location_to_grid(POSITION y, POSITION x) { return 256 * y + x; }

static projection_path_type *initialize_projection_path_type(
    projection_path_type *pp_ptr, ushort *gp, POSITION range, BIT_FLAGS flag, POSITION y1, POSITION x1, POSITION y2, POSITION x2)
{
    pp_ptr->gp = gp;
    pp_ptr->range = range;
    pp_ptr->flag = flag;
 ...
 * @return リストの長さ
 */
int projection_path(player_type *player_ptr, ushort *gp, POSITION range, POSITION y1, POSITION x1, POSITION y2, POSITION x2, BIT_FLAGS flag)
{
    if ((x1 == x2) && (y1 == y2))
        return 0;
 ...
 */
POSITION get_grid_y(ushort grid) { return (int)(grid / 256U); }

/*
 * Convert a "grid" (G) into a "location" (X)
 */
POSITION get_grid_x(ushort grid) { return (int)(grid % 256U); }
