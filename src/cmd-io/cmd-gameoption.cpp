...
static void do_cmd_options_autosave(PlayerType *player_ptr, concptr info)
{
    ...
    while (true) {
        ...
        case '?': {
            (void)show_file(player_ptr->name, true, _("joption.txt#Autosave", "option.txt#Autosave"), 0, 0);
            term_clear();
            break;
        }
...
static void do_cmd_options_win(PlayerType *player_ptr)
{
    ...
    while (go) {
        ...
        case 'S':
            ...
            break;
        case '?':
            (void)show_file(player_ptr->name, true, _("joption.txt#Window", "option.txt#Window"), 0, 0);
            term_clear();
            break;
        default:
            d = get_keymap_dir(ch);
    ...
    while (true) {
        ...
        case '4':
            *cheat_info[k].o_var = false;
            ...
            break;
        case '?':
            (void)show_file(player_ptr->name, true, std::string(_("joption.txt#", "option.txt#")).append(cheat_info[k].o_text), 0, 0);
            term_clear();
            break;
        default:
            bell();
...
void do_cmd_options(PlayerType *player_ptr)
{
    ...
    while (true) {
        ...
        case 'h': {
            ...
            while (true) {
                ...
                if (k == ESCAPE) {
                    ...
                    (void)show_file(player_ptr->name, true, _("joption.txt#Hitpoint", "option.txt#Hitpoint"), 0, 0);
                    term_clear();
                } else if (isdigit(k)) {
                    hitpoint_warn = D2I(k);
                } else {
        ...
        case 'm': {
            ...
            while (true) {
                ...
                if (k == ESCAPE) {
                    ...
                    (void)show_file(player_ptr->name, true, _("joption.txt#Manapoint", "option.txt#Manapoint"), 0, 0);
                    term_clear();
                } else if (isdigit(k)) {
                    mana_warn = D2I(k);
                } else {
        ...
        case '?':
            (void)show_file(player_ptr->name, true, _("joption.txt", "option.txt"), 0, 0);
            term_clear();
            break;
        default: {
            bell();
...
void do_cmd_options_aux(PlayerType *player_ptr, game_option_types page, concptr info)
{
    ...
    while (true) {
        ...
        case '?': {
            (void)show_file(player_ptr->name, true, std::string(_("joption.txt#", "option.txt#")).append(option_info[opt[k]].o_text), 0, 0);
            term_clear();
            break;
        }
        default: {
