...
static void display_player_exp(PlayerType *player_ptr)
{
    ...
    if (player_ptr->exp >= player_ptr->max_exp) {
        display_player_one_line(e, format("%ld", static_cast<long>(player_ptr->exp)), TERM_L_GREEN);
    } else {
        display_player_one_line(e, format("%ld", static_cast<long>(player_ptr->exp)), TERM_YELLOW);
    }

    if (!pr.equals(PlayerRaceType::ANDROID)) {
        display_player_one_line(ENTRY_MAX_EXP, format("%ld", static_cast<long>(player_ptr->max_exp)), TERM_L_GREEN);
    }
    ...
    if (player_ptr->lev >= PY_MAX_LEVEL) {
        ...
        display_player_one_line(e, format("%ld", (player_exp_a[player_ptr->lev - 1] * player_ptr->expfact / 100L)), TERM_L_GREEN);
    } else {
        display_player_one_line(e, format("%ld", (player_exp[player_ptr->lev - 1] * player_ptr->expfact / 100L)), TERM_L_GREEN);
    }
}
...
static void display_real_playtime(void)
{
    ...
    display_player_one_line(ENTRY_PLAY_TIME, format("%.2lu:%.2lu:%.2lu", static_cast<unsigned long>(play_hour), static_cast<unsigned long>(play_min), static_cast<unsigned long>(play_sec)), TERM_L_GREEN);
}
...
void display_player_middle(PlayerType *player_ptr)
{
    ...
    display_player_one_line(ENTRY_GOLD, format("%ld", static_cast<long>(player_ptr->au)), TERM_L_GREEN);
