...
COMMAND_CODE show_inventory(player_type *owner_ptr, int target_item, BIT_FLAGS mode, tval_type tval, const ItemTester& item_tester)
{
    ...
    prepare_label_string(owner_ptr, inven_label, USE_INVEN, tval, item_tester);
    for (k = 0, i = 0; i < z; i++) {
        o_ptr = &owner_ptr->inventory_list[i];
 ...
 */
void display_inventory(player_type *owner_ptr, tval_type tval, const ItemTester& item_tester)
{
    int i, n, z = 0;
    ...
    for (i = 0; i < z; i++) {
        ...
        auto o_ptr = &owner_ptr->inventory_list[i];
        auto do_disp = item_tester.okay(owner_ptr, o_ptr, tval);
        strcpy(tmp_val, "   ");
        if (do_disp) {
