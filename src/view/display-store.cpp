...
void display_entry(player_type *player_ptr, int pos)
{
    ...
    if ((cur_store_num == StoreSaleType::HOME) || (cur_store_num == StoreSaleType::MUSEUM)) {
        ...
        if (show_weights) {
            ...
            sprintf(out_val, _("%3d.%1d kg", "%3d.%d lb"), _(lb_to_kg_integer(wgt), wgt / 10), _(lb_to_kg_few(wgt), wgt % 10));
    ...
    if (show_weights) {
        ...
        sprintf(out_val, "%3d.%1d", _(lb_to_kg_integer(wgt), wgt / 10), _(lb_to_kg_few(wgt), wgt % 10));
