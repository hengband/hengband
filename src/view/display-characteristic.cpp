 ...
 * その行の表示色用の判定も行う
 */
static void process_cursed_equipment_characteristics(player_type *creature_ptr, ushort mode, char_stat &char_stat)
{
 ...
 * その行の表示色用の判定も行う
 */
static void process_light_equipment_characteristics(player_type *creature_ptr, all_player_flags *f, ushort mode, char_stat &char_stat)
{
    int max_i = (mode & DP_WP) ? INVEN_BOW + 1 : INVEN_TOTAL;
    for (int i = INVEN_MAIN_HAND; i < max_i; i++) {
        BIT_FLAGS flags[TR_FLAG_SIZE];
 ...
 * その行の表示色用の判定も行う
 */
static void process_inventory_characteristic(player_type *creature_ptr, tr_type flag, all_player_flags *f, ushort mode, char_stat &char_stat)
{
    int max_i = (mode & DP_WP) ? INVEN_BOW + 1 : INVEN_TOTAL;
    for (int i = INVEN_MAIN_HAND; i < max_i; i++) {
        BIT_FLAGS flags[TR_FLAG_SIZE];
 ...
 * @param mode 表示オプション
 */
static void process_one_characteristic(player_type *creature_ptr, TERM_LEN row, TERM_LEN col, std::string_view header, tr_type flag, all_player_flags *f, ushort mode)
{
    char_stat char_stat;
