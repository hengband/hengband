...
constexpr uint32_t QUEST_FLAG_SILENT = 0x01; /*!< クエストフラグ: クエスト進行に関する情報表示を抑止する / no messages from completion */
constexpr uint32_t QUEST_FLAG_PRESET = 0x02; /*!< クエストフラグ: クエストがダンジョン外で発生する / quest is outside the main dungeon */
constexpr uint32_t QUEST_FLAG_ONCE = 0x04; /*!< クエストフラグ: クエストがフロアを出た時点で完了する / quest is marked finished after leaving */
constexpr uint32_t QUEST_FLAG_TOWER = 0x08; /*!< クエストフラグ: クエスト:塔の形式で進行する / Tower quest is special */
...
public:
    ...
    int level = 0; /*!< 処理階層 / Dungeon level */
    MonraceId r_idx{}; /*!< クエスト対象のモンスターID / Monster race */

    int cur_num = 0; /*!< 撃破したモンスターの数 / Number killed */
    int max_num = 0; /*!< 求められるモンスターの撃破数 / Number required */

    int num_mon = 0; /*!< QuestKindTypeがKILL_NUMBER時の目標撃破数 number of monsters on level */

    uint32_t flags = 0; /*!< クエストに関するフラグビット / quest flags */
    DungeonId dungeon{}; /*!< クエスト対象のダンジョンID / quest dungeon */

    short complev = 0; /*!< クリア時プレイヤーレベル / player level (complete) */
    uint32_t comptime = 0; /*!< クリア時ゲーム時間 /  quest clear time*/
