...
std::string get_version()
{
    ...
    if (VERSION_STATUS == VersionStatusType::RELEASE) {
        ...
        return format(_("変愚蛮怒 %d.%d.%d.%d(%s)", "Hengband %d.%d.%d.%d(%s)"), H_VER_MAJOR, H_VER_MINOR, H_VER_STATE, H_VER_BUILD, mode);
    }

    return format(_("変愚蛮怒 %d.%d.%d%s%d", "Hengband %d.%d.%d%s%d"), H_VER_MAJOR, H_VER_MINOR, H_VER_STATE, expr.data(), H_VER_BUILD);
}
