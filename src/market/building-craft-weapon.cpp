...
PRICE compare_weapons(player_type *customer_ptr, PRICE bcost)
{
    ...
    OBJECT_IDX item;
    o_ptr[0] = choose_object(customer_ptr, &item, q, s, (USE_EQUIP | USE_INVEN | IGNORE_BOTHHAND_SLOT), TV_NONE, ItemTester(object_is_orthodox_melee_weapons));
    if (!o_ptr[0]) {
        screen_load();
        return 0;
    }
    ...
    while (true) {
        ...
        OBJECT_IDX item2;
        object_type *i2_ptr = choose_object(customer_ptr, &item2, q, s, (USE_EQUIP | USE_INVEN | IGNORE_BOTHHAND_SLOT), TV_NONE, ItemTester(object_is_orthodox_melee_weapons));
        if (!i2_ptr)
            continue;
