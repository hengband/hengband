...
void building_recharge(player_type *player_ptr)
{
    ...
    if (o_ptr->tval == ItemPrimaryType::TV_WAND && (o_ptr->pval / o_ptr->number >= k_ptr->pval)) {
        if (o_ptr->number > 1) {
        ...
        return;
    } else if (o_ptr->tval == ItemPrimaryType::TV_STAFF && o_ptr->pval >= k_ptr->pval) {
        if (o_ptr->number > 1) {
    ...
    if (o_ptr->tval == ItemPrimaryType::TV_ROD) {
        ...
        int max_charges;
        if (o_ptr->tval == ItemPrimaryType::TV_STAFF)
...
void building_recharge_all(player_type *player_ptr)
{
    ...
    for (INVENTORY_IDX i = 0; i < INVEN_PACK; i++) {
        ...
        if ((o_ptr->tval < ItemPrimaryType::TV_STAFF) || (o_ptr->tval > ItemPrimaryType::TV_ROD))
            continue;
        if (!o_ptr->is_known())
            total_cost += 50;
