...
static PRICE repair_broken_weapon_aux(player_type *player_ptr, PRICE bcost)
{
    ...
    if (o_ptr->sval == SV_BROKEN_DAGGER) {
        ...
        for (const auto &k_ref : k_info) {
            if (k_ref.tval != ItemPrimaryType::TV_SWORD)
                continue;
