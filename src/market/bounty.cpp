...
bool exchange_cash(player_type *player_ptr)
{
    ...
    for (INVENTORY_IDX i = 0; i <= INVEN_SUB_HAND; i++) {
        ...
        if ((o_ptr->tval == ItemPrimaryType::TV_CAPTURE) && (o_ptr->pval == MON_TSUCHINOKO)) {
    ...
    for (INVENTORY_IDX i = 0; i < INVEN_PACK; i++) {
        o_ptr = &player_ptr->inventory_list[i];
        if ((o_ptr->tval == ItemPrimaryType::TV_CORPSE) && (o_ptr->sval == SV_CORPSE) && (o_ptr->pval == MON_TSUCHINOKO)) {
            char buf[MAX_NLEN + 32];
            describe_flavor(player_ptr, o_name, o_ptr, 0);
            sprintf(buf, _("%s を換金しますか？", "Convert %s into money? "), o_name);
            if (get_check(buf)) {
    ...
    for (INVENTORY_IDX i = 0; i < INVEN_PACK; i++) {
        o_ptr = &player_ptr->inventory_list[i];
        if ((o_ptr->tval == ItemPrimaryType::TV_CORPSE) && (o_ptr->sval == SV_SKELETON) && (o_ptr->pval == MON_TSUCHINOKO)) {
            char buf[MAX_NLEN + 32];
            describe_flavor(player_ptr, o_name, o_ptr, 0);
            sprintf(buf, _("%s を換金しますか？", "Convert %s into money? "), o_name);
            if (get_check(buf)) {
    ...
    for (INVENTORY_IDX i = 0; i < INVEN_PACK; i++) {
        o_ptr = &player_ptr->inventory_list[i];
        if ((o_ptr->tval == ItemPrimaryType::TV_CORPSE) && (o_ptr->sval == SV_CORPSE)
    ...
    for (INVENTORY_IDX i = 0; i < INVEN_PACK; i++) {
        ...
        if ((o_ptr->tval == ItemPrimaryType::TV_CORPSE) && (o_ptr->sval == SV_SKELETON)
            && (streq(r_info[o_ptr->pval].name.c_str(), r_info[w_ptr->today_mon].name.c_str()))) {
            char buf[MAX_NLEN + 32];
            describe_flavor(player_ptr, o_name, o_ptr, 0);
            sprintf(buf, _("%s を換金しますか？", "Convert %s into money? "), o_name);
    ...
    for (int j = 0; j < MAX_BOUNTY; j++) {
        for (INVENTORY_IDX i = INVEN_PACK - 1; i >= 0; i--) {
            o_ptr = &player_ptr->inventory_list[i];
            if ((o_ptr->tval != ItemPrimaryType::TV_CORPSE) || (o_ptr->pval != w_ptr->bounty_r_idx[j]))
                continue;
