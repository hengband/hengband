...
errr init_feat_variables(void)
{
    ...
    for (i = 1; i < MAX_LJ_DOORS; i++) {
        short door = f_tag_to_index(format("LOCKED_DOOR_%d", i));
        if (door < 0)
            break;
    ...
    for (i = 0; i < MAX_LJ_DOORS; i++) {
        short door = f_tag_to_index(format("JAMMED_DOOR_%d", i));
        if (door < 0)
            break;
    ...
    /* Locked glass doors */
    for (i = 1; i < MAX_LJ_DOORS; i++) {
        short door = f_tag_to_index(format("LOCKED_GLASS_DOOR_%d", i));
        if (door < 0)
            break;
    ...
    /* Jammed glass doors */
    for (i = 0; i < MAX_LJ_DOORS; i++) {
        short door = f_tag_to_index(format("JAMMED_GLASS_DOOR_%d", i));
        if (door < 0)
            break;
