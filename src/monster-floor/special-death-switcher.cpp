...
static void summon_self(PlayerType *player_ptr, MonsterDeath *md_ptr, summon_type type, int probability, POSITION radius, concptr message)
{
    ...
    auto real_r_idx = (md_ptr->r_ptr->idx != md_ptr->ap_r_ptr->idx) ? tl::make_optional(md_ptr->r_ptr->idx) : tl::nullopt;
    if (summon_specific(player_ptr, m_pos.y, m_pos.x, 100, type, mode, tl::nullopt, real_r_idx) && player_can_see_bold(player_ptr, m_pos.y, m_pos.x)) {
        msg_print(message);
    }
}
