...
static void drop_artifact_from_unique(PlayerType *player_ptr, MonsterDeath *md_ptr)
{
    ...
    for (const auto &[fa_id, chance] : md_ptr->monrace->get_drop_artifacts()) {
...
static void drop_artifacts(PlayerType *player_ptr, MonsterDeath *md_ptr)
{
    ...
    if (md_ptr->monrace->misc_flags.has_not(MonsterMiscType::GUARDIAN) || (dungeon.final_guardian != md_ptr->m_ptr->r_idx)) {
        return;
    }
...
{
    AngbandWorld::get_instance().total_winner = true;
    InnerGameData::get_instance().add_winner_class(player_ptr->pclass);
...
void monster_death(PlayerType *player_ptr, MONSTER_IDX m_idx, bool drop_item, AttributeFlags attribute_flags)
{
    ...
    if (md.monrace->kind_flags.has(MonsterKindType::UNIQUE) && md.m_ptr->mflag2.has_not(MonsterConstantFlagType::CLONED)) {
        world.play_time.update();
        md.monrace->defeat_time = world.play_time.elapsed_sec();
        md.monrace->defeat_level = player_ptr->lev;
    }
