...
static void on_defeat_arena_monster(player_type *player_ptr, monster_death_type *md_ptr)
{
    ...
    if (arena_info[player_ptr->arena_number].tval > ItemPrimaryType::TV_NONE) {
        object_type forge;
...
static void drop_corpse(player_type *player_ptr, monster_death_type *md_ptr)
{
    ...
    object_type forge;
    object_type *q_ptr = &forge;
    q_ptr->prep(lookup_kind(ItemPrimaryType::TV_CORPSE, (corpse ? SV_CORPSE : SV_SKELETON)));
    apply_magic_to_object(player_ptr, q_ptr, floor_ptr->object_level, AM_NO_FIXED_ART);
...
{
    auto k_idx = d_info[player_ptr->dungeon_idx].final_object != 0 ? d_info[player_ptr->dungeon_idx].final_object : lookup_kind(ItemPrimaryType::TV_SCROLL, SV_SCROLL_ACQUIREMENT);
