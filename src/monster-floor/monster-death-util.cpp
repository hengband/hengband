    ...
    , m_ptr(&floor.m_list[m_idx])
{
    ...
    this->drop_chosen_item = drop_item && !this->cloned && !floor.inside_arena && !AngbandSystem::get_instance().is_phase_out() && !this->m_ptr->is_pet() && !this->m_ptr->mflag2.has(MonsterConstantFlagType::CHAMELEON);
}
