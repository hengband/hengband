 ...
 * @param type 召喚種別
 * @param unique_okay ユニークモンスターを召喚対象に含めるかどうか
 * @return 召喚対象にできるならばTRUE
 */
static bool summon_specific_okay(PlayerType *player_ptr, MonsterRaceId r_idx, MONSTER_IDX summon_who, summon_type type, bool unique_okay)
{
    ...
    if (!unique_okay && (r_ptr->kind_flags.has(MonsterKindType::UNIQUE) || (r_ptr->population_flags.has(MonsterPopulationType::NAZGUL)))) {
        return false;
    }
...
bool summon_specific(PlayerType *player_ptr, MONSTER_IDX src_idx, POSITION y1, POSITION x1, DEPTH lev, summon_type type, BIT_FLAGS mode)
{
    ...
    auto summon_specific_hook = [src_idx, type, unique_okay = (mode & PM_ALLOW_UNIQUE) != 0](PlayerType *player_ptr, MonsterRaceId r_idx) {
        return summon_specific_okay(player_ptr, r_idx, src_idx, type, unique_okay);
    };
