    ...
    if (!world.timewalk_m_idx) {
        ...
        for (int i = 1; i < floor.m_max; i++) {
            ...
            if (rad > 0) {
                ...
                should_lite &= (m_ptr->is_asleep() || (!floor.is_in_underground() && world.is_daytime()) || AngbandSystem::get_instance().is_phase_out());
                if (should_lite) {
                    continue;
                }
                ...
                if (r_ptr->brightness_flags.has_none_of({ MonsterBrightnessType::SELF_DARK_1, MonsterBrightnessType::SELF_DARK_2 }) && (m_ptr->is_asleep() || (!floor.is_in_underground() && !world.is_daytime()))) {
                    continue;
                }
