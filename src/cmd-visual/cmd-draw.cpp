...
void do_cmd_player_status(PlayerType *player_ptr)
{
    ...
    while (true) {
        ...
        if (c == 'c') {
            ...
            const auto filename_opt = get_string(_("ファイル名: ", "File name: "), 80, initial_filename);
            if (filename_opt.has_value()) {
                auto filename = str_ltrim(filename_opt.value());
                if (!filename.empty()) {
                    update_playtime();
                    file_character(player_ptr, filename.data());
                }
            }
        } else if (c == 'h') {
            mode++;
