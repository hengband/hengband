...
void call_the_void(PlayerType *player_ptr)
{
    ...
    for (int i = 0; i < 9; i++) {
        ...
        if (!grid.cave_has_flag(TerrainCharacteristics::PROJECT)) {
            if (!grid.mimic || grid.get_apparent_terrain(TerrainKind::MIMIC_RAW).flags.has_not(TerrainCharacteristics::PROJECT) || !grid.get_apparent_terrain().is_permanent_wall()) {
                do_call = false;
                break;
            }
        }
...
static void erase_wall(FloorType &floor, const Pos2D &pos)
{
    ...
    const auto &terrain = grid.get_apparent_terrain(TerrainKind::MIMIC_RAW);
    ...
    const auto &terrain_changed = grid.get_apparent_terrain(TerrainKind::MIMIC_RAW);
...
bool vanish_dungeon(PlayerType *player_ptr)
{
    ...
    for (auto y = 1; y < floor.height - 1; y++) {
        for (auto x = 1; x < floor.width - 1; x++) {
            const Pos2D pos(y, x);
            auto &grid = floor.get_grid(pos);
            const auto &terrrain = grid.get_apparent_terrain();
            grid.info &= ~(CAVE_ROOM | CAVE_ICKY);
