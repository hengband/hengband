...
#ifdef JP
    msg_format("あなたは%sを壁に近すぎる場所で唱えてしまった！", ((mp_ptr->spell_book == ItemPrimaryType::TV_LIFE_BOOK) ? "祈り" : "呪文"));
#else
    msg_format("You %s the %s too close to a wall!", ((mp_ptr->spell_book == ItemPrimaryType::TV_LIFE_BOOK) ? "recite" : "cast"),
        ((mp_ptr->spell_book == ItemPrimaryType::TV_LIFE_BOOK) ? "prayer" : "spell"));
#endif
