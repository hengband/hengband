...
void do_cmd_open(PlayerType *player_ptr)
{
    ...
    if (get_rep_dir(player_ptr, &dir, true)) {
        ...
        if (grid.get_apparent_terrain(TerrainKind::MIMIC).flags.has_not(TerrainCharacteristics::OPEN) && !o_idx) {
...
void do_cmd_close(PlayerType *player_ptr)
{
    ...
    if (get_rep_dir(player_ptr, &dir)) {
        const auto pos = player_ptr->get_neighbor(dir);
        const auto &grid = floor.get_grid(pos);
        if (grid.get_apparent_terrain(TerrainKind::MIMIC).flags.has_not(TerrainCharacteristics::CLOSE)) {
...
void do_cmd_bash(PlayerType *player_ptr)
{
    ...
    if (get_rep_dir(player_ptr, &dir)) {
        ...
        if (grid.get_apparent_terrain(TerrainKind::MIMIC).flags.has_not(TerrainCharacteristics::BASH)) {
