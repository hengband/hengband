...
static concptr item_activation_aux(object_type *o_ptr)
{
    ...
    case RandomArtActType::BR_FIRE:
        if ((o_ptr->tval == ItemPrimaryType::TV_RING) && (o_ptr->sval == SV_RING_FLAMES))
            desc = _("火炎のブレス (200) と火への耐性", "breathe fire (200) and resist fire");
        break;
    case RandomArtActType::BR_COLD:
        if ((o_ptr->tval == ItemPrimaryType::TV_RING) && (o_ptr->sval == SV_RING_ICE))
    ...
    if (constant == 0 && dice == 0) {
        ...
        switch (act_ptr->index) {
        case RandomArtActType::BR_FIRE:
            sprintf(timeout, _("%d ターン毎", "every %d turns"), ((o_ptr->tval == ItemPrimaryType::TV_RING) && (o_ptr->sval == SV_RING_FLAMES)) ? 200 : 250);
            break;
        case RandomArtActType::BR_COLD:
            sprintf(timeout, _("%d ターン毎", "every %d turns"), ((o_ptr->tval == ItemPrimaryType::TV_RING) && (o_ptr->sval == SV_RING_ICE)) ? 200 : 250);
            break;
...
concptr activation_explanation(object_type *o_ptr)
{
    ...
    if (o_ptr->tval == ItemPrimaryType::TV_WHISTLE) {
...
{
    if (book_tval < ItemPrimaryType::TV_LIFE_BOOK)
        return false;
    if (player_ptr->pclass == CLASS_SORCERER) {
        ...
        if (is_magic(tval2realm(enum2i(book_tval))))
            return ((book_tval == ItemPrimaryType::TV_ARCANE_BOOK) || (book_sval < 2));
    }
...
TERM_COLOR object_attr(object_type *o_ptr)
{
    return ((k_info[o_ptr->k_idx].flavor)
            ...
            : ((!o_ptr->k_idx || (o_ptr->tval != ItemPrimaryType::TV_CORPSE) || (o_ptr->sval != SV_CORPSE) || (k_info[o_ptr->k_idx].x_attr != TERM_DARK))
