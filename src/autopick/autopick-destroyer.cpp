...
static bool is_leave_special_item(player_type *player_ptr, object_type *o_ptr)
{
    ...
    if (player_ptr->prace == PlayerRaceType::BALROG) {
        if (o_ptr->tval == ItemPrimaryType::TV_CORPSE && o_ptr->sval == SV_CORPSE && angband_strchr("pht", r_info[o_ptr->pval].d_char))
            return false;
    } else if (player_ptr->pclass == CLASS_ARCHER) {
        if (o_ptr->tval == ItemPrimaryType::TV_SKELETON || (o_ptr->tval == ItemPrimaryType::TV_CORPSE && o_ptr->sval == SV_SKELETON))
            return false;
    } else if (player_ptr->pclass == CLASS_NINJA) {
        if (o_ptr->tval == ItemPrimaryType::TV_LITE && o_ptr->name2 == EGO_LITE_DARKNESS && o_ptr->is_known())
            return false;
    } else if (player_ptr->pclass == CLASS_BEASTMASTER || player_ptr->pclass == CLASS_CAVALRY) {
        if (o_ptr->tval == ItemPrimaryType::TV_WAND && o_ptr->sval == SV_WAND_HEAL_MONSTER && o_ptr->is_aware())
            return false;
    }
...
static bool is_opt_confirm_destroy(player_type *player_ptr, object_type *o_ptr)
{
    ...
    if (leave_chest)
        if ((o_ptr->tval == ItemPrimaryType::TV_CHEST) && o_ptr->pval)
            return false;
    ...
    if (leave_corpse)
        if (o_ptr->tval == ItemPrimaryType::TV_CORPSE)
            return false;

    if (leave_junk)
        if ((o_ptr->tval == ItemPrimaryType::TV_SKELETON) || (o_ptr->tval == ItemPrimaryType::TV_BOTTLE) || (o_ptr->tval == ItemPrimaryType::TV_JUNK) || (o_ptr->tval == ItemPrimaryType::TV_STATUE))
            return false;
