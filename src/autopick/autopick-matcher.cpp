...
bool is_autopick_match(player_type *player_ptr, object_type *o_ptr, autopick_type *entry, concptr o_name)
{
    ...
    if (IS_FLG(FLG_UNIQUE) && ((o_ptr->tval != ItemPrimaryType::TV_CORPSE && o_ptr->tval != ItemPrimaryType::TV_STATUE) || !(r_info[o_ptr->pval].flags1 & RF1_UNIQUE)))
        return false;

    if (IS_FLG(FLG_HUMAN) && (o_ptr->tval != ItemPrimaryType::TV_CORPSE || !angband_strchr("pht", r_info[o_ptr->pval].d_char)))
        return false;

    if (IS_FLG(FLG_UNREADABLE) && (o_ptr->tval < ItemPrimaryType::TV_LIFE_BOOK || check_book_realm(player_ptr, o_ptr->tval, o_ptr->sval)))
        return false;
    ...
    if (IS_FLG(FLG_FIRST) && ((o_ptr->tval < ItemPrimaryType::TV_LIFE_BOOK) || (o_ptr->sval) != 0))
        return false;

    if (IS_FLG(FLG_SECOND) && ((o_ptr->tval < ItemPrimaryType::TV_LIFE_BOOK) || (o_ptr->sval) != 1))
        return false;

    if (IS_FLG(FLG_THIRD) && ((o_ptr->tval < ItemPrimaryType::TV_LIFE_BOOK) || (o_ptr->sval) != 2))
        return false;

    if (IS_FLG(FLG_FOURTH) && ((o_ptr->tval < ItemPrimaryType::TV_LIFE_BOOK) || (o_ptr->sval) != 3))
        return false;
    ...
    if (IS_FLG(FLG_WEAPONS)) {
        ...
        if (o_ptr->tval != ItemPrimaryType::TV_CORPSE && o_ptr->tval != ItemPrimaryType::TV_SKELETON)
            return false;
    } else if (IS_FLG(FLG_SPELLBOOKS)) {
        if (!(o_ptr->tval >= ItemPrimaryType::TV_LIFE_BOOK))
            return false;
    } else if (IS_FLG(FLG_HAFTED)) {
        if (!(o_ptr->tval == ItemPrimaryType::TV_HAFTED))
            return false;
    } else if (IS_FLG(FLG_SHIELDS)) {
        if (!(o_ptr->tval == ItemPrimaryType::TV_SHIELD))
            return false;
    } else if (IS_FLG(FLG_BOWS)) {
        if (!(o_ptr->tval == ItemPrimaryType::TV_BOW))
            return false;
    } else if (IS_FLG(FLG_RINGS)) {
        if (!(o_ptr->tval == ItemPrimaryType::TV_RING))
            return false;
    } else if (IS_FLG(FLG_AMULETS)) {
        if (!(o_ptr->tval == ItemPrimaryType::TV_AMULET))
            return false;
    } else if (IS_FLG(FLG_SUITS)) {
        if (!(o_ptr->tval == ItemPrimaryType::TV_DRAG_ARMOR || o_ptr->tval == ItemPrimaryType::TV_HARD_ARMOR || o_ptr->tval == ItemPrimaryType::TV_SOFT_ARMOR))
            return false;
    } else if (IS_FLG(FLG_CLOAKS)) {
        if (!(o_ptr->tval == ItemPrimaryType::TV_CLOAK))
            return false;
    } else if (IS_FLG(FLG_HELMS)) {
        if (!(o_ptr->tval == ItemPrimaryType::TV_CROWN || o_ptr->tval == ItemPrimaryType::TV_HELM))
            return false;
    } else if (IS_FLG(FLG_GLOVES)) {
        if (!(o_ptr->tval == ItemPrimaryType::TV_GLOVES))
            return false;
    } else if (IS_FLG(FLG_BOOTS)) {
        if (!(o_ptr->tval == ItemPrimaryType::TV_BOOTS))
            return false;
    }
