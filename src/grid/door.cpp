 ...
 */
void place_secret_door(PlayerType *player_ptr, const Pos2D &pos, DoorKind door_kind)
{
    ...
    if (door_kind == DoorKind::DEFAULT) {
        door_kind = (dungeon.flags.has(DungeonFeatureType::CURTAIN) && one_in_(dungeon.flags.has(DungeonFeatureType::NO_CAVE) ? 16 : 256))
                        ? DoorKind::CURTAIN
...
void place_random_door(PlayerType *player_ptr, const Pos2D &pos, bool is_room_door)
{
    ...
    if (tmp < 300) {
        tag = feat_door.at(type).open;
    } else if (tmp < 400) {
        tag = feat_door.at(type).broken;
    } else if (tmp < 600) {
        place_closed_door(player_ptr, pos, type);
...
void place_closed_door(PlayerType *player_ptr, const Pos2D &pos, DoorKind door_kind)
{
    ...
    auto tag = TerrainTag::NONE;
    if (tmp < 300) {
        tag = feat_door.at(door_kind).closed;
    } else if (tmp < 399) {
        tag = feat_locked_door_random(door_kind);
    } else {
        tag = feat_jammed_door_random(door_kind);
    }
