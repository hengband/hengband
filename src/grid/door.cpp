...
{
    floor_type *floor_ptr = player_ptr->current_floor_ptr;
    if (d_info[static_cast<int>(floor_ptr->dungeon_idx)].flags.has(DF::NO_DOORS)) {
        place_bold(player_ptr, y, x, GB_FLOOR);
        return;
    }

    if (type == DOOR_DEFAULT) {
        type = (d_info[static_cast<int>(floor_ptr->dungeon_idx)].flags.has(DF::CURTAIN) && one_in_(d_info[static_cast<int>(floor_ptr->dungeon_idx)].flags.has(DF::NO_CAVE) ? 16 : 256))
            ? DOOR_CURTAIN
            : (d_info[static_cast<int>(floor_ptr->dungeon_idx)].flags.has(DF::GLASS_DOOR) ? DOOR_GLASS_DOOR : DOOR_DOOR);
    }
...
void place_locked_door(player_type *player_ptr, POSITION y, POSITION x)
{
    ...
    set_cave_feat(floor_ptr, y, x, feat_locked_door_random(d_info[static_cast<int>(player_ptr->dungeon_idx)].flags.has(DF::GLASS_DOOR) ? DOOR_GLASS_DOOR : DOOR_DOOR));
...
void place_random_door(player_type *player_ptr, POSITION y, POSITION x, bool room)
{
    ...
    int type = (d_info[static_cast<int>(floor_ptr->dungeon_idx)].flags.has(DF::CURTAIN) && one_in_(d_info[static_cast<int>(floor_ptr->dungeon_idx)].flags.has(DF::NO_CAVE) ? 16 : 256))
        ? DOOR_CURTAIN
        : (d_info[static_cast<int>(floor_ptr->dungeon_idx)].flags.has(DF::GLASS_DOOR) ? DOOR_GLASS_DOOR : DOOR_DOOR);
