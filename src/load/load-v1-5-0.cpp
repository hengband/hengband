...
void rd_item_old(object_type *o_ptr)
{
    ...
    rd_byte(&o_ptr->xtra1);
    rd_byte(&tmp8u);
    o_ptr->xtra2 = i2enum<RandomArtActType>(tmp8u);

    if (h_older_than(1, 0, 10)) {
        if (o_ptr->xtra1 == EGO_XTRA_SUSTAIN) {
            switch (enum2i(o_ptr->xtra2) % 6) {
            case 0:
            ...
            o_ptr->xtra2 = i2enum<RandomArtActType>(0);
        } else if (o_ptr->xtra1 == EGO_XTRA_POWER) {
            switch (enum2i(o_ptr->xtra2) % 11) {
            case 0:
