...
void rd_item_old(object_type *o_ptr)
{
    ...
    /* Type/Subtype */
    rd_byte(&tmp8u);
    o_ptr->tval = i2enum<ItemPrimaryType>(tmp8u);
    rd_byte(&tmp8u);
    o_ptr->sval = tmp8u;

    if (h_older_than(0, 4, 4)) {
        if (o_ptr->tval == i2enum <ItemPrimaryType>(100))
            o_ptr->tval = ItemPrimaryType::TV_GOLD;
        if (o_ptr->tval == i2enum<ItemPrimaryType>(98))
            o_ptr->tval = ItemPrimaryType::TV_MUSIC_BOOK;
        if (o_ptr->tval == i2enum<ItemPrimaryType>(110))
            o_ptr->tval = ItemPrimaryType::TV_HISSATSU_BOOK;
    }
    ...
    if (h_older_than(0, 2, 3)) {
        ...
        o_ptr->xtra4 = 0;
        o_ptr->xtra5 = 0;
        if ((o_ptr->tval == ItemPrimaryType::TV_CHEST) || (o_ptr->tval == ItemPrimaryType::TV_CAPTURE)) {
    ...
    if (h_older_than(1, 0, 5)
        && (((o_ptr->tval == ItemPrimaryType::TV_LITE) && ((o_ptr->sval == SV_LITE_TORCH) || (o_ptr->sval == SV_LITE_LANTERN))) || (o_ptr->tval == ItemPrimaryType::TV_FLASK))) {
    ...
    if (h_older_than(0, 4, 10) && (o_ptr->name2 == EGO_TWILIGHT))
        o_ptr->k_idx = lookup_kind(ItemPrimaryType::TV_SOFT_ARMOR, SV_TWILIGHT_ROBE);
