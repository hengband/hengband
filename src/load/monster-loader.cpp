...
void rd_monster(player_type *player_ptr, monster_type *m_ptr)
{
    ...
    if (any_bits(flags, SaveDataMonsterFlagType::SAVE_MON_AP_R_IDX))
        rd_s16b(&m_ptr->ap_r_idx);
    else
        m_ptr->ap_r_idx = m_ptr->r_idx;

    if (any_bits(flags, SaveDataMonsterFlagType::SAVE_MON_SUB_ALIGN))
        rd_byte(&m_ptr->sub_align);
    else
        m_ptr->sub_align = 0;

    if (any_bits(flags, SaveDataMonsterFlagType::SAVE_MON_CSLEEP))
    ...
    if (any_bits(flags, SaveDataMonsterFlagType::SAVE_MON_FAST)) {
        rd_byte(&tmp8u);
        m_ptr->mtimed[MTIMED_FAST] = (int16_t)tmp8u;
    } else
        m_ptr->mtimed[MTIMED_FAST] = 0;

    if (any_bits(flags, SaveDataMonsterFlagType::SAVE_MON_SLOW)) {
        rd_byte(&tmp8u);
        m_ptr->mtimed[MTIMED_SLOW] = (int16_t)tmp8u;
    } else
        m_ptr->mtimed[MTIMED_SLOW] = 0;

    if (any_bits(flags, SaveDataMonsterFlagType::SAVE_MON_STUNNED)) {
        rd_byte(&tmp8u);
        m_ptr->mtimed[MTIMED_STUNNED] = (int16_t)tmp8u;
    } else
        m_ptr->mtimed[MTIMED_STUNNED] = 0;

    if (any_bits(flags, SaveDataMonsterFlagType::SAVE_MON_CONFUSED)) {
        rd_byte(&tmp8u);
        m_ptr->mtimed[MTIMED_CONFUSED] = (int16_t)tmp8u;
    } else
        m_ptr->mtimed[MTIMED_CONFUSED] = 0;

    if (any_bits(flags, SaveDataMonsterFlagType::SAVE_MON_MONFEAR)) {
        rd_byte(&tmp8u);
        m_ptr->mtimed[MTIMED_MONFEAR] = (int16_t)tmp8u;
    } else
        m_ptr->mtimed[MTIMED_MONFEAR] = 0;

    if (any_bits(flags, SaveDataMonsterFlagType::SAVE_MON_TARGET_Y)) {
        rd_s16b(&tmp16s);
        m_ptr->target_y = (POSITION)tmp16s;
    } else
        m_ptr->target_y = 0;

    if (any_bits(flags, SaveDataMonsterFlagType::SAVE_MON_TARGET_X)) {
        rd_s16b(&tmp16s);
        m_ptr->target_x = (POSITION)tmp16s;
    } else
        m_ptr->target_x = 0;

    if (any_bits(flags, SaveDataMonsterFlagType::SAVE_MON_INVULNER)) {
        rd_byte(&tmp8u);
    ...
    if (any_bits(flags, SaveDataMonsterFlagType::SAVE_MON_SMART)) {
    ...
    if (any_bits(flags, SaveDataMonsterFlagType::SAVE_MON_EXP)) {
        uint32_t tmp32u;
        rd_u32b(&tmp32u);
    ...
    if (any_bits(flags, SaveDataMonsterFlagType::SAVE_MON_MFLAG2)) {
        if (loading_savefile_version_is_older_than(2)) {
            rd_byte(&tmp8u);
    ...
    if (any_bits(flags, SaveDataMonsterFlagType::SAVE_MON_NICKNAME)) {
        char buf[128];
    ...
    if (any_bits(flags, SaveDataMonsterFlagType::SAVE_MON_PARENT))
