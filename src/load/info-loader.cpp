...
void rd_version_info(void)
{
    ...
    if (tmp_major == variant_length) {
        ...
        system.version_state = rd_byte();
        system.version_build = rd_byte();
        strip_bytes(1);
    } else if (is_old_ver) {
        strip_bytes(3);
    } else {
    ...
    load_note(format(fmt, system.version_major, system.version_minor, system.version_state, loading_savefile_version));
}
