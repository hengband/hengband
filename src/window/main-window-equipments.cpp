...
COMMAND_CODE show_equipment(player_type *owner_ptr, int target_item, BIT_FLAGS mode, tval_type tval, const ItemTester& item_tester)
{
    ...
    for (k = 0, i = INVEN_MAIN_HAND; i < INVEN_TOTAL; i++) {
        ...
        if (!(owner_ptr->select_ring_slot ? is_ring_slot(i) : item_tester.okay(owner_ptr, o_ptr, tval) || (mode & USE_FULL))
    ...
    prepare_label_string(owner_ptr, equip_label, USE_EQUIP, tval, item_tester);
    for (j = 0; j < k; j++) {
        i = out_index[j];
        o_ptr = &owner_ptr->inventory_list[i];
