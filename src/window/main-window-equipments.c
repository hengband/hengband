...
COMMAND_CODE show_equipment(player_type *owner_ptr, int target_item, BIT_FLAGS mode, tval_type tval)
{
    ...
    for (k = 0, i = INVEN_RARM; i < INVEN_TOTAL; i++) {
        ...
        if (!(select_ring_slot ? is_ring_slot(i) : item_tester_okay(owner_ptr, o_ptr, tval) || (mode & USE_FULL))
            && (!((((i == INVEN_RARM) && have_left_hand_weapon(owner_ptr)) || ((i == INVEN_LARM) && have_right_hand_weapon(owner_ptr))) && owner_ptr->two_handed_weapon)
                || (mode & IGNORE_BOTHHAND_SLOT)))
            continue;

        describe_flavor(owner_ptr, o_name, o_ptr, 0);
        if ((((i == INVEN_RARM) && have_left_hand_weapon(owner_ptr)) || ((i == INVEN_LARM) && have_right_hand_weapon(owner_ptr))) && owner_ptr->two_handed_weapon) {
