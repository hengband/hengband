...
void fix_inventory(player_type *player_ptr, tval_type item_tester_tval)
{
    for (int j = 0; j < 8; j++) {
        ...
        display_inventory(player_ptr, item_tester_tval, ItemTester());
        term_fresh();
        term_activate(old);
    }
}
 ...
 */
static void display_equipment(player_type *owner_ptr, tval_type tval, const ItemTester& item_tester)
{
    ...
    for (int i = INVEN_MAIN_HAND; i < INVEN_TOTAL; i++) {
        ...
        auto do_disp = owner_ptr->select_ring_slot ? is_ring_slot(i) : item_tester.okay(owner_ptr, o_ptr, tval);
        strcpy(tmp_val, "   ");
...
void fix_equip(player_type *player_ptr, tval_type item_tester_tval)
{
    for (int j = 0; j < 8; j++) {
        ...
        term_activate(angband_term[j]);
        display_equipment(player_ptr, item_tester_tval, ItemTester());
        term_fresh();
        term_activate(old);
    }
}
