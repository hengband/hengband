/*!
 * @file unix-cfg-reader.cpp
 * @brief Windows版固有実装(.cfgファイル処理)のunix版
 * @details ほぼwin版のコピペだがiniファイルの呼び出しのためにSimpleIni(MITライセンス)を使用している
 */
...
CfgData CfgReader::read_sections(std::initializer_list<cfg_section> sections) const
{
    ...
    SI_Error rc = ini.LoadFile(this->cfg_path.c_str());
    if (rc < 0) {
        return result;
    }
