...
MONRACE_IDX get_mon_num(player_type *player_ptr, DEPTH min_level, DEPTH max_level, BIT_FLAGS option)
{
    ...
    if (d_info[static_cast<int>(player_ptr->dungeon_idx)].flags.has(DF::MAZE)) {
    ...
    /* Boost the max_level */
    if ((option & GMN_ARENA) || d_info[static_cast<int>(player_ptr->dungeon_idx)].flags.has_not(DF::BEGINNER)) {
...
void choose_new_monster(player_type *player_ptr, MONSTER_IDX m_idx, bool born, MONRACE_IDX r_idx)
{
    ...
    if (!r_idx) {
        ...
        if (d_info[static_cast<int>(player_ptr->dungeon_idx)].flags.has(DF::CHAMELEON))
            level += 2 + randint1(3);
